...
- **Bias–variance tradeoff.** Explicit, repeated guarantee. From Apr 28: *"Definitely going to be a question about bias variance in the test because I think that concept is the kind of the running theme through the course in all the models we looked at."* And **separately confirmed Feb 24** mid-semester: *"this is definitely going to be on the exam. I mean, just this concept... it's deceptively confusing. It seems really simple. It's not."* Read the bias-variance section of ISLR cover-to-cover. Be able to:
  ...
  - Discuss the over-parameterized / **double-descent / benign-overfitting** regime — instructor's favorite recent topic.
...
- **Effective DOF for smoothing spline:** **trace(S)** where S is the smoother matrix — this is *not* an integer in general.
...
- **Bagging variance under correlation ρ:** variance of average ≈ **ρσ² + (1−ρ)σ²/B** — does *not* shrink to 0 when ρ > 0. This is *the* reason RF decorrelates trees via mtry.
...
- **R²** = 1 − RSS/TSS. **Adjusted R²** = 1 − (RSS/(n−d−1)) / (TSS/(n−1)) — penalizes adding parameters.
...
| **Brain injury** | Bagging | Surprise: bagging didn't improve error (0.18 → 0.19) — nuance question |
...
12. **K-means random init.** Running once is wrong — the algorithm converges to local minima. Run multiple times, pick lowest within-cluster SS. Apr 14: *"random initialization can really screw you."*
...
**For QDA:** the x'Σ_k⁻¹x term does **not** cancel (Σ_k differs between classes), so the boundary is quadratic. The instructor explicitly flagged: *"where does the quadratic come from in QDA?"* — answer hinges on this cancellation failure.
...
- Pay attention to bias-variance, regularization, classification metrics, PCA — these recur every year.
...
8. **End-of-module review questions** in the slides — instructor draws inspiration from these for exam questions.
9. **Drill the worked-example datasets** (§4d). Pull the slides for Default, Boston Housing, Wage, Ozone, Brain Injury — re-do every interpretation prompt.
