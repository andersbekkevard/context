...
The book (`book/`) holds the **deep treatment** of any in-scope idea. The atom carries an `isl-ref:` pointer so Claude can tell Anders "for the full derivation, see ISLR §6.2.1." Atoms intentionally don't recreate textbook content — Claude already knows it from training, and the book is on the exam table for lookup.
...
- Other modules' MOCs and atoms wikilink back via `[[bias-variance]]` etc. — bidirectional linking makes the cross-cutting structure visible in the Obsidian graph
