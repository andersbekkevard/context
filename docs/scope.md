...
Anything in ISLR but NOT covered in lectures or exercises is OUT. When in doubt: grep `wiki/lectures/` and `exercises/` for the term — if absent, it's out.
...
**Not in scope** per the prof's rule — ISLR exercises are not the same as `exercises/`. But useful as *practice material* for hand-calculation fluency:
