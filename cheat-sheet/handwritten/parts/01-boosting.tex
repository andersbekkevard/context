...
\end{algorithmic}
\end{algorithm}
The $r_{im}$ are called \emph{generalised / pseudo-residuals}.
...
$p_{\text{drop}}$ (XGB) & random tree drop    & avoid early-tree dominance. \\
\bottomrule
\end{tabularx}
\normalsize
...
Subsample (without replacement) a fraction $\eta$ of training rows before fitting each tree. Typical $\eta=N/2$. Reduces variance \emph{and} computation. Variants: subsample rows, subsample columns per tree, subsample columns per split.
...
\item \textbf{Dropout} $p_{\text{drop}}$.
