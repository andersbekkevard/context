...
* The reducible error has to do with our estimate $\hat{f}$ of $f$. This error can be reduced by using the most _appropriate_ statistical learning technique. 
...
We simulated data according to the relationship between response $Y$ and covariate $x$
...
* **poly20**: Polynomial of degree 20 fit of the form $\beta_0+\beta_1 x +\beta_2 x^2+\cdots +\beta_{20}x^{20}$ _Overfits_ the data.
...
**A1**: If choosing flexibility based on training MSE: poly20 wins. Based on test MSE: poly2 wins.
...
$$Y  = f(x) + \varepsilon$$ 
to our training data $\{x_i, y_i\}$ for $i=1,..,n$, and $\varepsilon$ is an unobserved random variable that adds random, uncorrelated, mean-zero noise term with variance $\sigma^2$. ($\varepsilon$ is a substitute for all the unobserved variables that influence $Y$.)

* The training data was used to estimate $\hat{f}$.

* The _expected test mean squared error (MSE) at $x_0$_ (unseen test observation) is defined as:
...
* Compare this to the test MSE for the polynomial example ($\text{MSE}_{\text{test}}$): The average is simply replaced by the _theoretical version_ (expected value).
...
* _Irreducible error_. This term cannot be reduced regardless how well our statistical model fits the data. 

* _Variance_. of the prediction at $\hat{f}(x_0)$. Relates to the amount by which $\hat{f}(x_0)$ is expected to change for different training data. If the variance is high, there is large uncertainty associated with the prediction. 

* _Squared bias_. The bias gives an estimate of how much the prediction differs from the true mean. If the bias is low the model gives a prediction which is close to the true value.
...
This is called the _Bias-variance trade-off_.
...
* Aim: find the optimum where the _test MSE is minimal_.
