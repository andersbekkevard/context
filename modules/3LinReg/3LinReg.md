...
(From Theo Gasser & Burkhardt Seifert _Grundbegriffe der Biostatistik_)
...
Study with 243 male participants, where body fat (%) and BMI and other predictors were measured. Some scatterplots[^1]:

[^1]: The data to reproduce these plots and analyses can be found here: https://github.com/stefaniemuff/statlearning2/tree/master/3LinReg/data
...
For a good predictive model we need to dive into _multiple linear regression_. However, we start with the simple case of _only one predictor variable_:
...
* The model for $Y$ given $x$ has _three parameters_: $\beta_0$ (intercept), $\beta_1$ (slope coefficient) and $\sigma^2$ .
* $x$ is the _independent_/ _explanatory_ / _regressor_ variable.
* $Y$ is the _dependent_ / _outcome_ / _response_ variable.
...
* Mathematically, $a$ and $b$ are estimated such that the sum of _squared vertical distances_ (residual sum of squares)
...
Note: $\hat\beta_0$ and $\hat\beta_1$ are themselves _random variables_ and as such contain _uncertainty_!
...
* **Problem**: $\sigma$ is usually not known, but needs to be estimated[^2].
...
[^2]: $\sigma^2$ is the _irreducible error_ variance.
...
In words: $H_0$ =   "There is no relationship between $X$ and $Y$."[^3]

Here, the _alternative hypothesis_ is given by

$$H_A: \quad \beta_1 \neq  0  $$

[^3]: You could also test against another null hypothesis, like $\beta_1=c$.
...
There is **very strong evidence** that the BMI is associated with bodyfat. 
...
* Ideas to redefine what "statistical significance" means: [https://www.nature.com/articles/s41562-017-0189-z](https://www.nature.com/articles/s41562-017-0189-z)
...
* The $t$-distribution[^4] can be used to create confidence intervals for the regression parameters. The lower and upper limits of a 95% confidence interval for $\beta_j$ are
...
[^4]: If $n$ is large, the normal approximation to the $t$-distribution can be used (and is used in the textbook).

Doing this for the bodyfat example "by hand" is not hard. We have $241 (=243-2)$ degrees of freedom:
...
The interpretation of the coefficients $\beta_j$ is: holding all other covariates fixed, what is the average effect on $Y$ of a one-unit increase in the $j$th covariate.
...
In multiple linear regression, the parameter vector $\boldsymbol\beta$ is estimated with _maximum likelihood_ and _least squares_. These two methods give the same estimator when we assume the normal linear regression model. 
...
**The least squares and maximum likelihood estimator for ${\boldsymbol \beta}$:** is given like
...
From Module 2: Part B: $\mathbf{Y}_{(n\times 1)}$ with mean vector $\mathbf{\mu}$ and variance-covariance matrix $\Sigma$, then $\mathbf{Z}=\mathbf{C}\mathbf{Y}$ has $\mathsf{E}(\mathbf{Z})=\mathbf{C}\mathbf{\mu}$ and $\mathsf{Cov}(\mathbf{Z})=\mathbf{C}\Sigma\mathbf{C}^T$.

Also Module 2: Part B: If ${\bf Y}$ is multivariate normal, then also $\mathbf{C}\mathbf{Y}$ is multivariate normal.
...
See [here](https://www.math.ntnu.no/emner/TMA4268/2018v/notes/PropertiesBetahatMLR.pdf) for a derivation.
...
This leads to the _confidence interval_. 

2. **Where do future observations with a given $x$ coordinate lie?**

$\rightarrow$ Even if we could predict $\hat{Y}=f(X)$, the _true_ value $Y = f(X) + \varepsilon$ varies around $\hat{Y}$. We can compute a _prediction interval_ for new observations $Y$.
...
* To answer this question, we have to sum uncertainty over two components:
    ...
    1. the _uncertainty in the predicted value_ $\hat y_0 =  \hat\beta_0 + \hat\beta_1 x_1^{(0)} + \ldots \hat\beta_p x_p^{(0)}$ (due to uncertainty in $\hat{\boldsymbol\beta}$).

    2. the _irreducible error_ $\varepsilon_0 \sim N(0,\sigma^2)$.   
...
However: this model is _not identifiable_.[^5]

[^5]: What does that mean? I could add a constant to $\beta_1, \beta_2, ...\beta_k$ and subtract it from $\beta_0$, and the model would fit equally well to the data, so it cannot be decided which set of the parameters is best.
...
(Common aspect that leads to confusion!)
 ...
 _A factor covariate with $k$ factor levels requires $k-1$ parameters!_
...
$\rightarrow$ We again need the $F$-test[^6], as **always when we test for more than one $\beta_j=0$ _simultaneously_**!

[^6]: remember that the $F$-test is a generalization of the $t$-test!
...
* Even _transformations_ of covariates can be included in (almost) any form. For instance the square of a variable $X^2$
\begin{equation*}
...
The word _linear_ refers to the _linearity in the coefficients_, and not on a linear relationship between $Y$ and $X_1, \ldots , X_p$!
...
$\rightarrow$ In the later modules, we will discuss other more advanced non-linear approaches for addressing this issue.
...
To make valid inference from our model, we must check if our model assumptions are fulfilled![^7]
...
The _Tukey-Anscombe_ diagram plots the residuals against the fitted values. For the bodyfat data it looks like this:
...
<http://students.brown.edu/seeing-theory/regression-analysis/index.html> to do it manually![^8]

[^8]: You may choose "Ordinary Least Squares" and then click on "I" for the Anscombe quartet example. Drag points to see what happens with the regression line.
...
* Leverage is defined as the diagonal elements of the so-called _hat matrix_ $\mathbf{H}$[^9], i.e., the leverage of the $i$-th data point is $H_{ii}$ on the diagonal of $\mathbf{H = X(X^TX)^{-1}X^T}$. 
...
[^9]: Do you remember why $\mathbf{H}$ is called _hat matrix_?
...
$\rightarrow$ To make the $e_i$ more "like $\epsilon_i$", we use _standardized_ or _studentized residuals_.
...
* [Videos on YouTube by the authors of ISL, Chapter 3](https://www.youtube.com/playlist?list=PL5-da3qGB5IBSSCPANhTgrw82ws7w_or9)
