...
In classification we can split methods into two paradigms. Methods that model the class-conditional density $f_k(x) = \Pr(X \mid Y = k)$ and the prior $\pi_k$, then flip with Bayes' rule to obtain the posterior, are called \underline{\hspace{2.5cm}}\,(1) (\emph{generative} / \emph{discriminative} / \emph{semi-supervised} / \emph{kernel}) methods. Linear discriminant analysis is the canonical example. The decision boundary between two classes under LDA is obtained by setting the two discriminant scores $\delta_k(x)$ equal, and is \underline{\hspace{2.5cm}}\,(2) (\emph{linear} / \emph{quadratic} / \emph{piecewise constant} / \emph{circular}) in $x$, because the term $x^\top \Sigma^{-1} x$ cancels (it does not depend on $k$ when the covariance is \underline{\hspace{2.5cm}}\,(3) (\emph{pooled} / \emph{class-specific} / \emph{diagonal} / \emph{block-diagonal})).

When fitting a multiple linear regression with two predictors that are nearly proportional, the matrix $\mathbf X^\top \mathbf X$ becomes nearly singular and the variance of $\hat{\boldsymbol\beta}$ explodes. This phenomenon is called \underline{\hspace{2.5cm}}\,(4) (\emph{collinearity} / \emph{heteroscedasticity} / \emph{leverage} / \emph{influence}). The remedy proposed in the lectures that \emph{adds a penalty} $\lambda \sum_j \beta_j^2$ to the residual sum of squares without setting any coefficient exactly to zero is called \underline{\hspace{2.5cm}}\,(5) (\emph{ridge regression} / \emph{the lasso} / \emph{forward selection} / \emph{partial least squares}).

For tree ensembles, sampling, at each candidate split, a random subset of $m$ predictors of size $m < p$ is the trick that distinguishes \underline{\hspace{2.5cm}}\,(6) (\emph{random forests} / \emph{AdaBoost} / \emph{gradient boosting} / \emph{cost-complexity pruning}) from plain bagging. For each tree built on a bootstrap sample, roughly $37\%$ of the original observations are not in the sample, and the predictions on those left-out observations are aggregated into the so-called \underline{\hspace{2.5cm}}\,(7) (\emph{out-of-bag error} / \emph{validation error} / \emph{training error} / \emph{Bayes error}) estimate of test error.

In neural networks, the central optimization recipe is mini-batch stochastic gradient descent. The gradients with respect to all weights are computed efficiently by reusing intermediate values from the forward pass through repeated application of the chain rule; this algorithm is called \underline{\hspace{2.5cm}}\,(8) (\emph{backpropagation} / \emph{boosting} / \emph{the EM algorithm} / \emph{Newton-Raphson}). To prevent the network from becoming over-confident in its class predictions, especially when labels may themselves contain errors, one can replace the hard one-hot target by a slightly softened one; this technique is called \underline{\hspace{2.5cm}}\,(9) (\emph{label smoothing} / \emph{dropout} / \emph{early stopping} / \emph{batch normalization}).

Finally, the workhorse for estimating the standard error of a complicated statistic when no closed-form sampling distribution is available is to resample $n$ observations with replacement from the original sample many times, recompute the statistic on each resample, and take the sample standard deviation of the resulting values. This procedure is called the \underline{\hspace{2.5cm}}\,(10) (\emph{bootstrap} / \emph{$F$-test} / \emph{Bonferroni correction} / \emph{Newton step}).
...
\begin{enumerate}[(i)]
  ...
  \item \subpoints{1} Mark true or false: ``Compared to the unbiased choice $c = 1$, the shrunken estimator with $c < 1$ \emph{always} has a smaller mean squared error $\mathrm{Bias}^2 + \mathrm{Var}$, regardless of $\mu$ and $\sigma^2/n$.''
...
\begin{enumerate}[(i)]
  ...
  \item A typical practical learning rate for mini-batch SGD on a neural network is $\eta \approx 0.1$; a learning rate of $\eta = 2$ is generally too large and would cause the loss to bounce around without converging.
...
\begin{enumerate}[(i)]
  ...
  \item \subpoints{1} Mark true or false: ``Adding $L_2$ weight decay always decreases the network's training error.''
\end{enumerate}
