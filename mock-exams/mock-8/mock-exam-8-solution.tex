...
computation);
(9) ``dropout'' (dropout zeros activations, not targets).}
...
written un-squared. (ii) the common trap is to say ``True --- ridge always helps,'' which conflates
the qualitative bias--variance message with a literal-always claim. (iii) the trap is to confuse the
\emph{shape} of variance-vs-flexibility with the validity of the identity.}
...
\begin{enumerate}[(i)]
  ...
  \item \textbf{True}. The course-stated practical range is $\eta \in [10^{-3}, 10^{-1}]$ with
        $0.01$--$0.1$ typical; $\eta = 2$ would overshoot every basin and oscillate without
        converging.
...
\noindent\emph{Grading: 1\,P each. The (ii) trap is to think ``larger = faster'' --- the prof
explicitly cited divergent-loss screenshots at large $\eta$.}
...
\begin{enumerate}[(i)]
  ...
  \item \subpoints{1} \textbf{True}. Each tree's effective contribution to the ensemble is
        $\nu\cdot\hat T_m$, so halving $\nu$ roughly doubles the trees needed to reach the same
        cumulative fit. This is the well-known $(M, \nu)$ joint trade-off; the rule of thumb is
...
interpretation. The trap on (iii) is the easy reverse misreading: ``halving $\nu$ halves $M$,''
which gets the qualitative direction backward.}
...
\textbf{(B) True.} For $m$ i.i.d.\ summands, $\mathrm{Var}(\widehat{\nabla L}) \propto 1/m$, so
doubling $m$ halves the variance, all else equal.
...
epoch). (ii) 1\,P for the full T/T/T pattern. (iii) 1\,P for any defensible
