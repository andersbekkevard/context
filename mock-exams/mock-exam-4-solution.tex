...
\begin{enumerate}[(i)]
  ...
  \item \textbf{False}. With $L = \tfrac{1}{2}(y_i-\hat f(x_i))^2$,
        $\partial L/\partial \hat f = -(y_i-\hat f(x_i))$, so the backward-pass seed is the
        \emph{negative} residual, $\delta^{\text{out}}_i = -(y_i-\hat f(x_i))$. The sign matters:
        backprop pushes the gradient of the loss, not the residual itself.
\end{enumerate}
...
\begin{enumerate}[(i)]
  ...
  \item \textbf{True}. The prof's typical default is $\sim 20\%$ dropout for fully-connected
        hidden layers on tabular data; she explicitly flagged $50\%$ as too aggressive on
        signal-poor tabular problems (it tends to under-fit). The statement just records both
        facts together.
