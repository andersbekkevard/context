...
\begin{enumerate}[(i)]
  ...
  \item \subpoints{1} \textbf{True}. With $K$ dummies and an intercept the dummy columns sum
        to the intercept column, producing exact rank deficiency. The standard remedy is to drop
        one dummy (reference encoding), or equivalently to drop the intercept.
...
\begin{enumerate}[(i)]
  ...
  \item \subpoints{1} \textbf{True}. The prof gave $d \in \{1,\dots,8\}$ as the canonical range,
        with stumps as a special case; deep trees defeat the ``many weak learners'' principle by
        making each base learner already low-bias and high-variance, which boosting cannot easily
        unwind.
