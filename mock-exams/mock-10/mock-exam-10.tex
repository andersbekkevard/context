...
In supervised learning we have an outcome $Y$ and predictors $X$. A method that assumes a specific functional form $f(x;\theta)$ for the regression function and fits a small, finite set of parameters $\theta$ is called \underline{\hspace{2.5cm}}\,(1) (\emph{parametric} / \emph{nonparametric} / \emph{generative} / \emph{ensemble}). When the goal is to forecast the response on new inputs we speak of \underline{\hspace{2.5cm}}\,(2) (\emph{clustering} / \emph{calibration} / \emph{prediction} / \emph{inference}); when instead the goal is to understand which inputs drive the response and by how much, we speak of \underline{\hspace{2.5cm}}\,(3) (\emph{prediction} / \emph{inference} / \emph{boosting} / \emph{cross-validation}).

A central decomposition of expected test error at a fixed test point $x_0$ splits it as $\mathrm{Bias}^2 + \mathrm{Var} + \sigma^2$, where the last term, the variance of the noise on the response, is called the \underline{\hspace{2.5cm}}\,(4) (\emph{reducible} / \emph{Bayes-optimal} / \emph{cross-validated} / \emph{irreducible}) error. The classical wisdom relating the first two terms is the bias--variance \emph{trade-off}; in the over-parameterized regime where the number of parameters far exceeds $n$ and yet test error keeps falling past the interpolation threshold, the phenomenon is called \underline{\hspace{2.5cm}}\,(5) (\emph{shrinkage} / \emph{double descent} / \emph{stacking} / \emph{cold start}).

Among classifiers, methods that model $\Pr(X \mid Y)$ together with a prior $\pi_k = \Pr(Y = k)$ and then invert with Bayes' rule are called \underline{\hspace{2.5cm}}\,(6) (\emph{discriminative} / \emph{nonparametric} / \emph{generative} / \emph{shrinkage}) classifiers. The canonical example in this course assumes the class-conditional densities are Gaussian with a \emph{shared} covariance matrix, giving a decision boundary that is linear in $x$; this method is called \underline{\hspace{2.5cm}}\,(7) (\emph{QDA} / \emph{logistic regression} / \emph{naive Bayes} / \emph{LDA}).

In tree ensembles, growing many \emph{independent} bootstrap-replicated trees in parallel and averaging them is called \underline{\hspace{2.5cm}}\,(8) (\emph{bagging} / \emph{boosting} / \emph{stacking} / \emph{pruning}); growing many small trees \emph{sequentially}, each one a small correction to the previous ensemble, is called \underline{\hspace{2.5cm}}\,(9) (\emph{bagging} / \emph{boosting} / \emph{cost-complexity pruning} / \emph{stratified sampling}). When the goal is both to \emph{select} a hyperparameter and to obtain an \emph{honest} estimate of test error from the same training data, the appropriate resampling procedure is \underline{\hspace{2.5cm}}\,(10) (\emph{stratified cross-validation} / \emph{validation-set approach} / \emph{nested cross-validation} / \emph{the bootstrap}).
...
\begin{enumerate}[(i)]
  ...
  \item \subpoints{1} True or false: ``Encoding a categorical predictor with $K$ levels using $K$ dummy variables \emph{in addition to} an intercept column produces an exactly collinear design (`dummy-variable trap'); the standard remedy is to add a small ridge penalty to the OLS objective, which restores identifiability of all $K$ dummy coefficients individually.''
...
\begin{enumerate}[(i)]
  ...
  \item \subpoints{1} True or false: ``A reasonable default for the interaction depth of trees in gradient boosting is $d \in \{10, 20, \dots, 50\}$ --- deep trees that each capture rich, high-order interactions; using stumps ($d = 1$) would be too restrictive and defeat the bias--variance logic that motivates boosting.''
