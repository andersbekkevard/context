...
\begin{enumerate}[(i)]
  ...
  \item \subpoints{1} \textbf{False}. Early stopping uses the \emph{validation} error, not the
        training error: training is halted at the epoch where validation error first stops
        improving (or starts rising), and the weights from that epoch are returned. Stopping at
        the training-error minimum would never stop --- training error typically decreases
        monotonically --- and would entirely miss the moment when generalization begins to
        deteriorate, which is the whole point of early stopping.
\end{enumerate}
...
\begin{enumerate}[(i)]
  ...
  \item \subpoints{1} \textbf{False}. Boosting reduces \emph{bias} relative to a single shallow
        tree, by fitting trees sequentially to the residuals (or negative gradient) of the current
        ensemble; the trees are \emph{not} fit on independent bootstrap replicates, and they are
        \emph{not} simply averaged --- they are added in a stagewise sum with shrinkage. Variance
        reduction by bootstrap averaging is the mechanism of \emph{bagging} and random forests,
        not boosting.
...
\begin{enumerate}[(i)]
  ...
  \item \textbf{False}. The test error of a random forest is \emph{monotonically non-increasing}
        in $B$ (averaging only reduces variance, never causes overfitting), so the curve is not
        U-shaped and $B$ is \emph{not} tuned by CV. One simply picks ``enough'' trees --- typically
        $500$--$1000$. The standard contrast: in \emph{boosting}, $B$ \emph{is} a real tuning
        parameter because boosting can overfit.
\end{enumerate}
