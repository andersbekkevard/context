...
\begin{enumerate}[(i)]
  ...
  \item \textbf{False}. L2 weight decay penalizes large weights, which forces the optimizer toward
        a less expressive solution and generally \emph{increases} training error. Its purpose is
        to reduce \emph{test} error (variance reduction at the cost of some bias).
\end{enumerate}

\subsection*{e) Mini-batch SGD and backpropagation \subpoints{3}}
\sol{} \emph{(1\,P per statement.)}
\begin{enumerate}[(i)]
  \item \textbf{True}. Backpropagation is exactly the chain-rule application: the forward pass
        stores intermediate activations, and the backward pass uses them to compute
        $\partial L/\partial \theta$ for every parameter $\theta$ in time proportional to the
        forward pass. It does not \emph{do} the gradient step; that is the optimizer's job.
