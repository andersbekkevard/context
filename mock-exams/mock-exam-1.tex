...
Which of the following are true?
\begin{enumerate}[(i)]
  \item As $\lambda \to 0$, the fit approaches the rough interpolating spline that goes through (essentially) every training point.
...
\begin{enumerate}[(i)]
  ...
  \begin{itemize}
    ...
    \item[(B)] A classifier with AUC $= 0.95$ is necessarily better than one with AUC $= 0.85$ on every possible threshold.
...
\begin{enumerate}[(i)]
  ...
  \item it reduces the total number of parameters;
  \item it forces the network to be fully connected.
\end{enumerate}
...
\begin{tikzpicture}[scale=0.85]
  ...
  \draw[smooth, thick, blue] plot[domain=0.4:7.6] (\x, {4.4 + 0.04*(\x-1)*(\x-1)*(\x-1) + 0.2*(\x-1)*(\x-1) - 0.6*(\x-1)});
  \draw[dashed] (1,0) -- (1, 3.9) node[above] {\small $\hat\lambda_{\min}$};
  \draw[dashed] (3,0) -- (3, 4.0) node[above] {\small $\hat\lambda_{1SE}$};
...
\begin{tikzpicture}[scale=2.0]
  ...
  \draw[dotted] (0,0) -- (1,1);
  \draw[smooth, thick, red] plot[domain=0:1] (\x, {1-(1-\x)*(1-\x)*0.9 - 0.05*(1-\x)*\x});
