...
In supervised statistical learning we typically distinguish between methods that produce an explicit functional form for the relationship between predictors and response, called \underline{\hspace{2.5cm}}\,(1) (\emph{generative} / \emph{parametric} / \emph{nonparametric} / \emph{regularized}) methods, and methods that estimate the relationship more flexibly without assuming such a form, called \underline{\hspace{2.5cm}}\,(2) (\emph{parametric} / \emph{nonparametric} / \emph{linear} / \emph{Bayesian}) methods.

A central concept of the course was that the expected squared test error of a fitted model can be written as the sum of three pieces: a squared bias, a variance, and an \underline{\hspace{2.5cm}}\,(3) (\emph{reducible} / \emph{regularized} / \emph{irreducible} / \emph{validated}) component that does not vanish as we collect more training data. Methods such as ridge regression and lasso are referred to as \underline{\hspace{2.5cm}}\,(4) (\emph{boosting} / \emph{generative} / \emph{shrinkage} / \emph{kernel}) methods because they shrink coefficients toward zero. Of these two, \underline{\hspace{2.5cm}}\,(5) (\emph{ridge} / \emph{lasso} / \emph{both} / \emph{neither}) can produce a model in which some coefficients are exactly zero, and is therefore the natural choice when one also wants automatic variable selection.

When the goal is to estimate the prediction error of a fitted model honestly, we use a resampling technique. The simplest is the validation-set approach; a more reliable variant that splits the training data into $k$ roughly equal pieces and rotates which piece serves as held-out data is called \underline{\hspace{2.5cm}}\,(6) (\emph{the bootstrap} / \emph{leave-one-out cross-validation} / \emph{nested CV} / \emph{k-fold cross-validation}). The lecturer in this course generally prefers this technique to penalty-based criteria such as AIC because the latter rely on \underline{\hspace{2.5cm}}\,(7) (\emph{shrinkage} / \emph{distributional assumptions} / \emph{regularization} / \emph{cross-validation}) that often fail in practice.

For non-linear regression we discussed methods that allow each predictor to enter the model through its own non-linear shape and are then summed; this family is called \underline{\hspace{2.5cm}}\,(8) (\emph{principal components} / \emph{convolutional networks} / \emph{boosted trees} / \emph{generalized additive models}).

Finally, in unsupervised learning we discussed two main families. One reduces the dimensionality of the data by finding orthogonal directions of maximal variance and is called \underline{\hspace{2.5cm}}\,(9) (\emph{lasso} / \emph{principal component analysis} / \emph{partial least squares} / \emph{neural networks}); another partitions the observations into groups, of which the agglomerative variant produces a tree-like structure called a \underline{\hspace{2.5cm}}\,(10) (\emph{biplot} / \emph{scree plot} / \emph{dendrogram} / \emph{decision tree}).
...
\begin{enumerate}[(i)]
  ...
  \item In the over-parameterized regime where the number of model parameters $p$ is much larger than $n$, a model that interpolates the training data has test error that grows without bound as $p/n$ increases.
...
\begin{enumerate}[(i)]
  ...
  \item In a cubic regression spline the knot locations enter the fit non-linearly and are estimated jointly with the spline coefficients by least squares.
\end{enumerate}
...
\begin{tikzpicture}[scale=0.85]
  ...
  \draw[smooth, thick, blue] plot[domain=0.4:7.6] (\x, {2.5 + 0.03*(\x-1.2)*(\x-1.2) + 0.002*(\x-1.2)*(\x-1.2)*(\x-1.2)});

  \draw[dashed] (1.2,0) -- (1.2,5.6);
  \node[above] at (1.2,5.6) {\small $\hat\lambda_{\min}$};
  \draw[dashed] (3.0,0) -- (3.0,5.6);
  \node[above] at (3.0,5.6) {\small $\hat\lambda_{1\mathrm{SE}}$};
...
\begin{tikzpicture}[scale=2.0]
  ...
  \draw[dotted] (0,0) -- (1,1);
  \draw[smooth, thick, red] plot[domain=0:1] (\x, {1-(1-\x)*(1-\x)*(1-\x)*(1-\x)*(1-\x)});
