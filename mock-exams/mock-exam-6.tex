...
Statistical learning methods that assume a specific functional form $f(x;\theta)$ for the regression function and then estimate $\theta$ from data are called \underline{\hspace{2.5cm}}\,(1) (\emph{parametric} / \emph{nonparametric} / \emph{generative} / \emph{kernel}) methods. Within the supervised-learning framework, the goal of estimating $f$ so as to forecast the response on new inputs is called \underline{\hspace{2.5cm}}\,(2) (\emph{prediction} / \emph{inference} / \emph{classification} / \emph{clustering}), while the goal of understanding which inputs drive the response and by how much is called \underline{\hspace{2.5cm}}\,(3) (\emph{prediction} / \emph{inference} / \emph{regression} / \emph{bootstrap}).

The expected squared test error of a fitted regression model at a new point can always be written as the sum of a squared bias, a variance, and an \underline{\hspace{2.5cm}}\,(4) (\emph{irreducible} / \emph{reducible} / \emph{cross-validated} / \emph{biased}) component arising from the noise in the response itself. Among the two main shrinkage methods we discussed, \underline{\hspace{2.5cm}}\,(5) (\emph{ridge} / \emph{lasso} / \emph{both} / \emph{neither}) shrinks coefficients toward zero but, except in degenerate cases, never sets them exactly to zero.

To estimate the generalization error of a fitted model without using the test set, one applies a resampling procedure. When the goal is also to \emph{tune} a hyperparameter (e.g.\ the penalty $\lambda$) and then assess the resulting pipeline honestly, one must wrap the hyperparameter-selection step inside an outer resampling loop --- this is called \underline{\hspace{2.5cm}}\,(6) (\emph{nested cross-validation} / \emph{stratified cross-validation} / \emph{the bootstrap} / \emph{the validation-set approach}).

In the neural-network module we saw that updating the parameters using the gradient computed on a small random subset of training observations, rather than the full dataset, is called \underline{\hspace{2.5cm}}\,(7) (\emph{mini-batch stochastic gradient descent} / \emph{batch gradient descent} / \emph{Newton's method} / \emph{backpropagation through time}). Three regularization techniques specific to neural networks are: randomly zeroing a fraction of node outputs during each training pass, called \underline{\hspace{2.5cm}}\,(8) (\emph{dropout} / \emph{label smoothing} / \emph{early stopping} / \emph{batch normalization}); monitoring validation loss and halting training when it begins to rise, called \underline{\hspace{2.5cm}}\,(9) (\emph{early stopping} / \emph{dropout} / \emph{weight decay} / \emph{data augmentation}); and replacing the hard one-hot training targets by slightly softened versions to discourage overconfident predictions, called \underline{\hspace{2.5cm}}\,(10) (\emph{label smoothing} / \emph{dropout} / \emph{transfer learning} / \emph{batch normalization}).
...
\begin{enumerate}[(i)]
  ...
  \item In the over-parameterized regime ($p\gg n$), a model that fits the training data with zero residual error necessarily generalizes badly to new data.
