...
\begin{tikzpicture}[scale=0.85]
  ...
  \draw[->] (0,0) -- (0,5) node[above] {CV-MSE};

  \draw[smooth, thick, blue] plot[domain=0.3:7.7] (\x, {3.2 + 0.30*(\x-2.0)*(\x-2.0) - 0.10*(\x-2.0)});
  \draw[dashed] (2.0,0) -- (2.0, 3.0) node[above] {\small $\hat\lambda_{\min}$};
  \draw[dashed] (3.6,0) -- (3.6, 3.5) node[above] {\small $\hat\lambda_{1SE}$};
...
\begin{tikzpicture}[scale=2.0]
  ...
  \draw[dotted] (0,0) -- (1,1);
  \draw[smooth, thick, red] plot[domain=0:1] (\x, {1-(1-\x)*(1-\x)*0.85 - 0.07*(1-\x)*\x});
  \draw[fill=black] (0.13, 0.375) circle (0.015) node[right] {\scriptsize $\hat p = 0.5$};
  \draw[fill=black] (0.36, 0.66) circle (0.015) node[right] {\scriptsize $\hat p = 0.3$};
