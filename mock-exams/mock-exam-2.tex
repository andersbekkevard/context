...
In statistical learning we often partition the data into three disjoint pieces: a training set used to fit a candidate model, a \underline{\hspace{2cm}}\,(1) (\emph{bootstrap} / \emph{validation} / \emph{holdout-of-the-residuals} / \emph{oob}) set used to choose between candidate models or tune a hyperparameter, and a test set used to estimate the chosen model's final generalization error. When data are too scarce for a clean three-way split we replace the validation set by \underline{\hspace{2cm}}\,(2) (\emph{regularization} / \emph{permutation testing} / \emph{cross-validation} / \emph{label smoothing}); the most common choice in this course is $k$-fold with $k = 5$ or $10$.
...
For model selection within the regression module we distinguished discrete and continuous approaches. Best-subset selection considers all $2^p$ subsets of predictors, while \underline{\hspace{2cm}}\,(4) (\emph{lasso shrinkage} / \emph{principal component} / \emph{boosted} / \emph{forward stepwise}) selection performs a greedy search that adds one predictor at a time and is computationally tractable even when $p > n$. Among continuous approaches, ridge regression shrinks coefficients toward zero but never sets them exactly to zero, whereas \underline{\hspace{2cm}}\,(5) (\emph{ridge} / \emph{lasso} / \emph{PCR} / \emph{GAM}) can yield \underline{\hspace{2cm}}\,(6) (\emph{sparse} / \emph{dense} / \emph{orthogonal} / \emph{biased}) coefficient vectors because the geometry of its constraint set has corners on the coordinate axes.

For unsupervised analyses on the same data, dimensionality reduction can be performed by \underline{\hspace{2cm}}\,(7) (\emph{ridge regression} / \emph{the bootstrap} / \emph{the F-test} / \emph{principal component analysis}), which finds orthogonal directions of maximal variance. Clustering can be done by $K$-means (which always returns a flat partition into $K$ clusters and can converge to a \underline{\hspace{2cm}}\,(8) (\emph{global} / \emph{Bayes-optimal} / \emph{local} / \emph{deterministic}) minimum depending on initialization) or by hierarchical clustering, which produces a tree-structured nested family of clusterings called a \underline{\hspace{2cm}}\,(9) (\emph{biplot} / \emph{dendrogram} / \emph{scree plot} / \emph{ROC curve}).
...
\begin{enumerate}[(i)]
  ...
  \item For large $n$, the probability that a given training observation \emph{appears} in a particular bootstrap sample approaches $1/e \approx 0.368$.
  \item A common bootstrap-based $95\%$ confidence interval for $\hat\theta$ is the \emph{percentile interval}: the $2.5\%$ and $97.5\%$ quantiles of the bootstrap distribution $\{\hat\theta^*_1, \dots, \hat\theta^*_B\}$.
\end{enumerate}
...
Mark each statement true or false.
\begin{enumerate}[(i)]
  \item Given a fixed initialization, the $K$-means algorithm converges to the \emph{global} minimum of the within-cluster sum of squares.
  ...
  \item Going from $K = 4$ to $K = 5$ in $K$-means produces a $5$-cluster solution in which one of the original $4$ clusters is split, with the other three unchanged.
\end{enumerate}
