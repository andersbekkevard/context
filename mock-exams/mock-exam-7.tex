...
A central theme of this course is \underline{\hspace{2.5cm}}\,(1) (\emph{regularization} / \emph{interpolation} / \emph{model averaging} / \emph{the Bayes rule}): any modification of a learning algorithm that aims to reduce \underline{\hspace{2.5cm}}\,(2) (\emph{generalization error} / \emph{training error} / \emph{the intercept} / \emph{the residual variance}) without necessarily reducing training error.

In the linear regression module we discussed what happens when two predictors are highly correlated: the matrix $\mathbf{X}^\top\mathbf{X}$ becomes nearly singular, the variances of the estimated coefficients explode, and individual $t$-tests can become misleading. This phenomenon is called \underline{\hspace{2.5cm}}\,(3) (\emph{collinearity} / \emph{heteroscedasticity} / \emph{leverage} / \emph{overfitting}).

For training neural networks the standard optimizer is \underline{\hspace{2.5cm}}\,(4) (\emph{mini-batch stochastic gradient descent} / \emph{coordinate descent} / \emph{Newton--Raphson} / \emph{the EM algorithm}), and the gradients themselves are computed efficiently by an algorithm that reuses intermediates from the forward pass, namely \underline{\hspace{2.5cm}}\,(5) (\emph{backpropagation} / \emph{boosting} / \emph{bootstrapping} / \emph{cross-validation}).

A neural network with many more weights than observations needs regularization to generalize well. One classification-specific trick is \underline{\hspace{2.5cm}}\,(6) (\emph{label smoothing} / \emph{batch normalization} / \emph{Adam} / \emph{the universal approximation theorem}), in which the hard one-hot targets are replaced by slightly softened versions.

For tree ensembles, growing many \emph{independent} trees in parallel and averaging them describes \underline{\hspace{2.5cm}}\,(7) (\emph{bagging and random forests} / \emph{gradient boosting} / \emph{AdaBoost} / \emph{cost-complexity pruning}), whereas growing many small trees \emph{sequentially}, each one a small correction to the previous ensemble's predictions, describes \underline{\hspace{2.5cm}}\,(8) (\emph{boosting} / \emph{bagging} / \emph{stacking} / \emph{the bootstrap}).

When estimating the standard error of a complicated statistic for which no clean closed-form sampling distribution is available, the standard nonparametric tool in this course is the \underline{\hspace{2.5cm}}\,(9) (\emph{bootstrap} / \emph{F-test} / \emph{validation-set approach} / \emph{Mallows' $C_p$}).

Finally, when we want both to \emph{select} a hyperparameter and to \emph{honestly assess} the resulting model on the same data set, the recommended procedure is \underline{\hspace{2.5cm}}\,(10) (\emph{nested cross-validation} / \emph{the validation-set approach} / \emph{leave-one-out CV} / \emph{the bootstrap}).
...
\begin{enumerate}[(i)]
  ...
  \item \subpoints{1} True or false: ``Early stopping returns the network parameters from the epoch at which the \emph{training} error first stops decreasing.''
\end{enumerate}
...
\begin{enumerate}[(i)]
  ...
  \item \subpoints{1} True or false: ``Boosting reduces \emph{variance} relative to a single deep tree by averaging over many bootstrap replicates.''
...
\begin{enumerate}[(i)]
  ...
  \item The number of trees $B$ in a random forest is a tuning parameter that must be carefully chosen by cross-validation, because random forests can overfit if $B$ is too large.
\end{enumerate}
