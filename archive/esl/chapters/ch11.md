...
(PPR) model has the form
                        ...
                        f (X) =         gm (ωm X).                    (11.1)
                                  m=1
                                                              T
This is an additive model, but in the derived features Vm = ωm  X rather
than the inputs themselves. The functions gm are unspecified and are esti-
g(V )                                            g(V )
...
**FIGURE 11.1.** Perspective plots of two ridge functions.         √
 ...
 mated along with the directions ωm using some flexible smoothing method
 (see below).
    The function gm (ωm  T
                           X) is called a ridge function in IRp . It varies only
                                                                                T
 in the direction defined by the vector ωm . The scalar variable Vm = ωm          X
 is the projection of X onto the unit vector ωm , and we seek ωm so that
 ...
 We seek the approximate minimizers of the error function
                        ...
                        X                X
                                  ...
                                  yi −         gm (ωm xi )                (11.2)
                        i=1              m=1
over functions gm and direction vectors ωm , m = 1, 2, . . . , M . As in other
...
direction vector ω, we form the derived variables vi = ω T xi . Then we have
...
X                                  N
      ...
                         2              ′  T                  T        yi − g(ωoldT
                                                                                        xi )
                  T
          yi − g(ω xi )        ≈         g (ωold x i )2        ωold xi +         T x )
                                                                                                      T
                                                                                                   − ω xi        .
i=1                                i=1
                                                                           g ′ (ωold  i
    ...
    To minimize the right-hand side, we carry out a least squares regression
              T
with target ωold             T
                 xi +(yi −g(ωold xi ))/g ′ (ωold
                                             T
                                                 xi ) on the input xi , with weights
  ′  T      2
g (ωold xi ) and no intercept (bias) term. This produces the updated coef-
ficient vector ωnew .
...
stage-wise manner, adding a pair (ωm , gm ) at each stage.
    There are a number of implementation details.
      ...
      • Usually the ωm are not readjusted (partly to avoid excessive compu-
...
s = 21 (blue curve) and s = 10 (purple curve). The scale parameter s controls
the activation rate, and we can see that large s amounts to a hard activation at
v = 0. Note that σ(s(v − v0 )) shifts the activation threshold from 0 to v0 .
...
activation of the sigmoid depends on the norm of αm , and if kαm k is very
...
model as a PPR model, we identify
                                T                       T
                           gm (ωm X)    =   βm σ(α0m + αm X)
                                                              T
                                        =   βm σ(α0m + kαm k(ωm X)),        (11.7)

where ωm = αm /kαm k is the mth unit-vector. Since σβ,α0 ,s (v) = βσ(α0 +
...
For classification we use either squared error or cross-entropy (deviance):
                                 ...
                                 N X
                                 X
                      R(θ) = −              yik log fk (xi ),               (11.10)
                                 i=1 k=1

and the corresponding classifier is G(x) = argmaxk fk (x). With the softmax
...
network, keeping track only of quantities local to each unit.
  ...
  Here is back-propagation in detail for squared error loss. Let zmi =
          T
σ(α0m + αm  xi ), from (11.5) and let zi = (z1i , z2i , . . . , zM i ). Then we have
                                    N
                                    X
                       R(θ) ≡             Ri
                                    i=1
...
with derivatives
        ∂Ri
            = −2(yik − fk (xi ))gk′ (βkT zi )zmi ,
       ∂βkm
               XK                                                           (11.12)
        ∂Ri
            =−     2(yik − fk (xi ))gk′ (βkT zi )βkm σ ′ (αm
                                                           T
                                                             xi )xiℓ .
       ∂αmℓ
                   k=1
  ...
  Now write (11.12) as
                               ...
                               ∂βkm
                                ...
                                ∂Ri
                                     = smi xiℓ .
                               ∂αmℓ
The quantities δki and smi are “errors” from the current model at the
...
current weights are fixed and the predicted values fˆk (xi ) are computed
...
tion (Robbins
         P      and Munro,P1951); results in this field ensure convergence if
γr → 0, r γr = ∞, and r γr2 < ∞ (satisfied, for example, by γr = 1/r).
...
boundary). Both use the softmax activation function and cross-entropy error.
      ...
      1                                                           1
               ...
               z1   z2   z3   z1   z5   z6   z7   z8   z9   z10            z1   z2   z3   z1   z5   z6   z7   z8   z9   z10
...
bers of hidden units, and recorded the average test error ETest (Y − fˆ(X))2
...
11.8, we see that the test error is less sensitive to the value of the weight
                      ...
                      No Weight Decay                                               Weight Decay=0.1
             3.0                                                       3.0
...
0–9. The predicted value fˆk (x) represents the estimated probability that
...
σj2 , and prior mean zero. The posterior distributions for each variance σj2
...
i.i.d. samples. We can write all of these models in the form
                                   L
                                   X
                     fˆ(xnew ) =         wℓ E(Ynew |xnew , θ̂ℓ )      (11.21)
                                   ℓ=1

In all cases the θ̂ℓ are a large collection of model parameters. For the
...
wℓ = 1/L as well, and the θ̂ℓ are the parameters refit to bootstrap re-
...
strategy.
 ...
 Bagged neural networks. We used the same architecture as in the Bayesian
     ...
     11-10 release), and Matlab neural-net toolbox (version 5.1).
                           ...
                           Univariate Screened Features                                                ARD Reduced Features
...
Test Error (%)                                                          Test Error (%)
                  ...
                  5                                                                      5
                       ...
                       Arcene   Dexter   Dorothea   Gisette   Madelon                         Arcene    Dexter   Dorothea   Gisette   Madelon
...
show that the single-layer regression network is equivalent to a PPR model
           T                        T
with gm (ωm  x) = βm σ(α0m + sm (ωm   x)), where ωm is the mth unit vector.
