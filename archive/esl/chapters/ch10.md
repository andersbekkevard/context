...
**Algorithm 10.1 AdaBoost.M1.**
   ...
   2. For m = 1 to M :
      ...
      (d) Set wi ← wi · exp[αm · I(yi 6= Gm (xi ))], i = 1, 2, . . . , N .
                         hP                  i
                             M
   3. Output G(x) = sign         α
                             m=1 m m G   (x)   .
...
current classifier Gm (x) is induced on the weighted observations at line 2a.
...
a very weak classifier is illustrated in Figure 10.2. The features X1 , . . . , X10
are standard independent Gaussian, and the deterministic target Y is de-
fined by                            P10
                             1     if j=1 Xj2 > χ210 (0.5),
                    Y =                                                     (10.2)
                             −1 otherwise.
Here χ210 (0.5) = 9.34 is the median of a chi-squared random variable with
...
classifiers Gm (x) ∈ {−1, 1}. More generally, basis function expansions take
the form
                                   M
                                   X
                           f (x) =    βm b(x; γm ),                   (10.3)
...
techniques covered in this book:
   • In single-hidden-layer neural networks (Chapter 11), b(x; γ) = σ(γ0 +
     γ1T x), where σ(t) = 1/(1 + e−t ) is the sigmoid function, and γ param-
...
on the ith observation. Thus, for squared-error loss, the term βm b(x; γm )
...
step. This can be expressed as
       ...
       (m)                                                   (m)
with wi     = exp(−yi fm−1 (xi )). Since each wi      depends neither on β
...
of β > 0, the solution to (10.9) for Gm (x) is
                                       ...
                                       N
                                       X        (m)
                     Gm = arg min            wi       I(yi 6= G(xi )),          (10.10)
                                   G
                                       i=1
...
which in turn can be written as
                                                N
                  NX (m)                       X (m)
         eβ − e−β ·  wi I(yi 6= G(xi )) + e−β ·   wi .                       (10.11)
                       i=1                                             i=1
...
Using the fact that −yi Gm (xi ) = 2 · I(yi 6= Gm (xi )) − 1, (10.14) becomes
                   (m+1)             (m)
                  wi          = wi         · eαm I(yi 6=Gm (xi )) · e−βm ,   (10.15)
where αm = 2βm is the quantity defined at line 2c of AdaBoost.M1 (Al-
gorithm 10.1). The factor e−βm in (10.15) multiplies all weights by the
...
interpreting f as the logit transform. Let
        ...
        p(x) = Pr(Y = 1 | x) =                       =                    (10.17)
                                  e−f (x) + ef (x)       1 + e−2f (x)
and define Y ′ = (Y + 1)/2 ∈ {0, 1}. Then the binomial log-likelihood loss
function is
...
margin yi f (xi ) > 0 are classified correctly whereas those with negative
margin yi f (xi ) < 0 are misclassified. The decision boundary is defined by
...
responding risk on the population is
 ...
 f ∗ (x) = arg min EY |x (Y −f (x))2 = E(Y | x) = 2·Pr(Y = 1 | x)−1. (10.19)
               f (x)
...
gin values yi f (xi ) > 1 it increases quadratically, thereby placing increasing
...
classified yi f (xi ) < 0. Thus, if class assignment is the goal, a monotone de-
...
set G = {G1 , . . . , Gk } (see Sections 2.4 and 4.4). We now seek a classifier
...
fK (x) = 0, as in (4.17). Here we prefer to retain the symmetry, and impose
                PK
the constraint k=1 fk (x) = 0. The binomial deviance extends naturally
...
output values yi , with weights wi = exp(−yi fm−1 (xi )). Using other more
...
with parameters Θ = {Rj , γj }J1 . J is usually treated as a meta-parameter.
...
mization problem into two parts:
 ...
 Finding Rj : This is the difficult part, for which approximate solutions are
     ...
     optimizing the Rj :
      ...
      Then given the R̂j = R̃j , the γj can be estimated more precisely
...
the forward stagewise procedure one must solve
          ...
          Θ̂m = arg min          L (yi , fm−1 (xi ) + T (xi ; Θm ))      (10.29)
                      Θm
                           i=1
for the region set and constants Θm = {Rjm , γjm }J1 m of the next tree, given
...
residuals yi − fm−1 (xi ), and γ̂jm is the mean of these residuals in each
...
classification trees, then we showed in Section 10.4 that the solution to
                                                              PN     (m)
(10.29) is the tree that minimizes the weighted error rate i=1 wi I(yi 6=
                              (m)
T (xi ; Θm )) with weights wi     = e−yi fm−1 (xi ) . By a scaled classification
tree, we mean βm T (x; Θm ), with the restriction that γjm ∈ {−1, 1}).
...
Steepest descent chooses hm = −ρm gm where ρm is a scalar and gm ∈ IRN
...
given the current model fm−1 and its fits fm−1 (xi ). Thus, the tree predic-
...
by its ANOVA (analysis of variance) expansion
         X              X                       X
 η(X) =      ηj (Xj ) +        ηjk (Xj , Xk ) +   ηjkl (Xj , Xk , Xl ) + · · · . (10.40)
            j              jk                  jkl
...
identical. A typical value for η can be 21 , although for large N , η can be
...
Here Iℓk is the relevance of Xℓ in separating the class k observations from
...
clues, especially when f (x) is dominated by low-order interactions (10.40).
   Consider the subvector XS of ℓ < p of the input predictor variables X T =
...
which f¯S (XS ) is to be evaluated. This can be computationally intensive,

  1 lattice in R.

even for moderately sized data sets. Fortunately with decision trees, f¯S (XS )
...
(10.47) represent the effect of XS on f (X) after accounting for the (av-
...
alone. The quantities f˜S (XS ) and f¯S (XS ) will be the same only in the
...
Then (10.47) produces the h1 (XS ) up to an additive constant. If the effect
...
mar-dlinc                                                   age
        age                                          hsld-stat
              ...
              0   20      40    60       80   100                 0   20   40     60     80   100
                   ...
                   Class = Prof/Man                                   Class = Homemaker
...
Ex. 10.4
 ...
 (d) Change the setup of this example as follows: define two classes, with
     ...
     also
     P standard      independent Gaussian, but conditioned on the event
            2
        j X j > 12. Now  the classes have significant overlap in feature space.
...
sification problem, consider the coding Y = (Y1 , . . . , YK )T with
                            
                               1,        if G = Gk
                       Yk =        1                                 (10.55)
                               − K−1  , otherwise.
                                   PK
Let f = (f1 , . . . , fK )T with    k=1 fk = 0, and define
                                                    
                                           1
                           L(Y, f ) = exp − Y T f        .                (10.56)
                                           K
...
1 if observation i is in class k and zero P otherwise. Suppose we have a
                                              K
current model fk (x), k = 1, . . . , K, with k=1 fk (x) = 0 (see (10.21) in
            ...
            iii. Compute
                 ...
                 γjkm =         P                                , j = 1, 2, . . . , Jm .
                           K      xi ∈Rjkm |rikm |(1 − |rikm |)
                                                     ...
                                                     P Jm
            iv. Update fkm (x) = fk,m−1 (x) +           j=1 γjkm I(x ∈ Rjkm ).
