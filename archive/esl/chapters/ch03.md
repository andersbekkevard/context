...
the fitted values at the training inputs are
                          ŷ = Xβ̂ = X(XT X)−1 XT y,                          (3.7)

where ŷi = fˆ(xi ). The matrix H = X(XT X)−1 XT appearing in equation
...
residual vector y − ŷ is orthogonal to this subspace. This orthogonality is
...
to form tests of hypothesis and confidence intervals for the parameters βj .
...
large N , the quantiles of Fp1 −p0 ,N −p1 −1 approach those of χ2p1 −p0 /(p1 −p0 ).
...
entire parameter vector β, namely
          ...
          (1−α)
where χ2ℓ       is the 1 − α percentile of the chi-squared distribution on ℓ
                                      (1−0.05)             (1−0.1)
...
Then the expected prediction error of an estimate f˜(x0 ) = xT0 β̃ is
              E(Y0 − f˜(x0 ))2 = σ 2 + E(xT0 β̃ − f (x0 ))2
                                = σ 2 + MSE(f˜(x0 )).                           (3.22)
Therefore, expected prediction error and mean squared error differ only by
the constant σ 2 , representing the variance of the new observation y0 .
...
(the columns of the data matrix X) are orthogonal; that is hxj , xk i = 0
for all j 6= k. Then it is easy to check that the multiple least squares esti-
mates β̂j are equal to hxj , yi/hxj , xj i—the univariate estimates. In other
...
the least squares coefficient of x has the form
                           ...
                           β̂1 =                      ,                          (3.27)
                                   hx − x̄1, x − x̄1i
           P
where x̄ = i xi /N , and 1 = x0 , the vector of N ones. We can view the
...
on a with no intercept, producing coefficient γ̂ = ha, bi/ha, ai and residual
vector b − γ̂a. We say that b is adjusted for a, or is “orthogonalized” with
respect to a.
...
### Algorithm 3.1: Regression by Successive Orthogonalization.
     ...
     2. For j = 1, 2, . . . , p
          ...
          Regress xj on z0 , z1 , . . . , , zj−1 to produce coefficients γ̂ℓj =
             hzℓ , xj i/hzℓ , zℓ i, ℓ = 0, . . . , j − 1 and residual vector zj =
                    Pj−1
             xj − k=0 γ̂kj zk .
...
univariate regression coefficient of y on xj·012...(j−1)(j+1)...,p , the residual
...
zp will be close to zero, and from (3.28) the coefficient β̂p will be very
...
In other words, the precision with which we can estimate β̂p depends on
...
where Z has as columns the zj (in order), and Γ is the upper triangular ma-
trix with entries γ̂kj . Introducing the diagonal matrix D with jth diagonal
entry Djj = kzj k, we get
...
Hence the coefficients for the kth outcome are just the least squares es-
...
suppose Cov(ε) = Σ, then the multivariate weighted criterion
                               N
                               X
               RSS(B; Σ) =           (yi − f (xi ))T Σ−1 (yi − f (xi ))      (3.40)
                               i=1
...
ear regression problem Y = X T β + ε. There are N = 300 observations on p = 31
                              ...
                              X
                               N                   p
                                                   X                        p
                                                                            X        
                                                                  2
      β̂ ridge = argmin               yi − β0 −          xij βj        +λ         βj2 .      (3.41)
                      β         i=1                j=1                      j=1
...
into two parts, after reparametrization using centered inputs: each xij gets
                                                 PN
replaced by xij − x̄j . We estimate β0 by ȳ = N1 1 yi . The remaining co-
...
that the input matrix X has p (rather than p + 1) columns.
   Writing the criterion in (3.41) in matrix form,
                  ...
                  RSS(λ) = (y − Xβ)T (y − Xβ) + λβ T β,                (3.43)

the ridge regression solutions are easily seen to be

                     β̂ ridge = (XT X + λI)−1 XT y,                    (3.44)

where I is the p×p identity matrix. Notice that with the choice of quadratic
penalty β T β, the ridge regression solution is again a linear function of
...
cer example, plotted as functions of df(λ), the effective degrees of freedom
...
squares estimates, that is, β̂ ridge = β̂/(1 + λ).
...
pose yi ∼ N (β0 + xTi β, σ 2 ), and the parameters βj are each distributed as
N (0, τ 2 ), independently of one another. Then the (negative) log-posterior
density of β, with τ 2 and σ 2 assumed known, is equal to the expression
in curly braces in (3.41), with λ = σ 2 /τ 2 (Exercise 3.6). Thus the ridge
...
fitted vector as
                        ...
                        Xβ̂ ls     = X(XT X)−1 XT y
                                   = UUT y,                           (3.46)

after some simplification. Note that UT y are the coordinates of y with
...
where the uj are the columns of U. Note that since λ ≥ 0, we have d2j /(d2j +
...
by the factors d2j /(d2j + λ). This means that a greater amount of shrinkage
is applied to the coordinates of basis vectors with smaller d2j .
   What does a small value of d2j mean? The SVD of the centered matrix
...
sample variance is easily seen to be
                         ...
                         Var(z1 ) = Var(Xv1 ) =       ,               (3.49)
                                                  N
and in fact z1 = Xv1 = u1 d1 . The derived variable z1 is called the first
principal component of X, and hence u1 is the normalized first principal
...
ferences. The lasso estimate is defined by
                                              ...
                                              X                    p
                                                                   X              2
                     β̂ lasso   =   argmin           yi − β0 −           xij βj
                                        β      i=1                 j=1
                                                     p
                                                     X
                                    subject to             |βj | ≤ t.                  (3.51)
                                                     j=1
...
ture, the lasso is also known as basis pursuit (Chen et al., 1998).
  We can also write the lasso problem in the equivalent Lagrangian form
                            X N            Xp             Xp       
          lasso             1                         2
     β̂           = argmin        yi − β0 −     xij βj + λ     |βj | .                 (3.52)
                       β    2 i=1           j=1            j=1

Notice the similarity
                  Pp to the ridge regression problem (3.42) orPp(3.41): the
L2 ridge penalty 1 βj2 is replaced by the L1 lasso penalty 1 |βj |. This
...
cause some of the coefficients to be exactly zero. Thus the lasso Pp does a kind
of continuous subset selection. If t is chosen larger than t0 = 1 |β̂j | (where
β̂j = β̂jls , the least squares estimates), then the lasso estimates are the β̂j ’s.
On the other hand, for t = t0 /2 say, then the least squares coefficients are
...
expected prediction error.
   In Figure 3.7, for ease of interpretation, we have plotted the lasso  Pp pre-
diction error estimates versus the standardized parameter s = t/ 1 |β̂j |.
A value ŝ ≈ 0.36 was chosen by 10-fold cross-validation; this caused four
...
of Table 3.3) are fairly large.
   Figure 3.10 shows Pp the lasso coefficients as the standardized tuning pa-
rameter s = t/ 1 |β̂j | is varied. At s = 1.0 these are the least squares
...
coefficient by a constant factor λ, truncating at zero. This is called “soft
...
Coefficients are plotted versus s = t/ p1 |β̂j |. A vertical line is drawn at s = 0.36,
                                       P
...
areas are the constraint regions |β1 | + |β2 | ≤ t and β12 + β22 ≤ t2 , respectively,
while the red ellipses are the contours of the least squares error function.


region for ridge regression is the disk β12 + β22 ≤ t, while that for lasso is
the diamond |β1 | + |β2 | ≤ t. Both methods find the first point where the
...
estimates. Consider the criterion
                      (N               p               p
                       ...
                       X              X        2     X
                                                               q
          β̃ = argmin       yi − β0 −    xij βj + λ      |βj |          (3.53)
                β      i=1            j=1                 j=1
                                                  P
for q ≥ 0. The contours of constant value of j |βj |q are shown in Fig-
...
and the elastic-net penalty j (αβj2 +(1−α)|βj |) for α = 0.2 (right plot). Although
                            P
...
### Algorithm 3.2: Least Angle Regression.
     1. Standardize the predictors to have mean zero and unit norm. Start
        with the residual r = y − ȳ, β1 , β2 , . . . , βp = 0.

     2. Find the predictor xj most correlated with r.

     3. Move βj from 0 towards its least-squares coefficient hxj , ri, until some
...
rk = y − XAk βAk is the current residual, then the direction for this step is
                              δk = (XTAk XAk )−1 XTAk rk .                             (3.55)
The coefficient profile then evolves as βAk (α) = βAk + α · δk . Exercise 3.23
...
this step is f̂k , then it evolves as f̂k (α) = f̂k + α · uk , where uk = XAk δk
...
function of their L1 arc length 2 . Note that we do not need to take small
              ...
              1 arc-length of a differentiable curve β(s) for s ∈ [0, S] is given by TV(β, S) =
RS
 0  ||β̇(s)||1 ds, where β̇(s) = ∂β(s)/∂s. For the piecewise-linear LAR coefficient profile,
...
where sj ∈ {−1, 1} indicates the sign of the inner-product, and γ is the
common value. Also |xTk (y − Xβ)| ≤ γ ∀k 6∈ A. Now consider the lasso
...
give
                    xTj (y − Xβ) = λ · sign(βj ), ∀j ∈ B              (3.58)
Comparing (3.58) with (3.56), we see that they are identical only if the
sign of βj matches the sign of the inner product. That is why the LAR

algorithm and lasso start to differ when an active coefficient passes through
...
vector ŷ = (ŷ1 , ŷ2 , . . . , ŷN ) as
                                       ...
                                       N
                                  1 X
                       df(ŷ) =           Cov(ŷi , yi ).                      (3.60)
                                  σ 2 i=1

Here Cov(ŷi , yi ) refers to the sampling covariance between the predicted
...
can be applied to any model prediction ŷ. This includes models that are
                                                                 ...
                                                                 Lasso
                                                                 Forward Stagewise
0.65                                                             Incremental Forward Stagewise
...
that df(ŷ) = k. Likewise for ridge regression, this definition leads to the
closed-form expression (3.50) on page 68: df(ŷ) = tr(Sλ ). In both these
cases, (3.60) is simple to evaluate because the fit ŷ = Hλ y is linear in y.
...
verified by estimating Cov(ŷi , yi )/σ 2 directly by simulation. However there
...
are orthogonal, this regression is just a sum of univariate regressions:
                                            ...
                                            X
                           pcr
                         ŷ(M  ) = ȳ1 +          θ̂m zm ,                 (3.61)
                                            m=1

where θ̂m = hzm , yi/hzm , zm i. Since the zm are each linear combinations
...
cipal components, using shrinkage factors d2j /(d2j + λ) as in (3.47). Principal
...
putingP ϕ̂1j = hxj , yi for each j. From this we construct the derived input
z1 =      j ϕ̂1j xj , which is the first partial least squares direction. Hence
...
Algorithm 3.3.
   ...
   3 Since the x are standardized, the first directions ϕ̂
                 j                                          1j are the univariate regression
coefficients (up to an irrelevant constant); this is not the case for subsequent directions.

### Algorithm 3.3: Partial Least Squares.
  1. Standardize each xj to have mean zero and variance one. Set ŷ(0) =
               (0)
     ȳ1, and xj = xj , j = 1, . . . , p.

  2. For m = 1, 2, . . . , p
               Pp            (m−1)                  (m−1)
      (a) zm = j=1 ϕ̂mj xj         , where ϕ̂mj = hxj     , yi.
      (b) θ̂m = hzm , yi/hzm , zm i.
      (c) ŷ(m) = ŷ(m−1) + θ̂m zm .
                                    (m−1)                         (m)      (m−1)
      (d) Orthogonalize each xj             with respect to zm : xj     = xj       −
                   (m−1)
           [hzm , xj     i/hzm , zm i]zm , j = 1, 2, . . . , p.

  3. Output the sequence of fitted vectors {ŷ(m) }p1 . Since the {zℓ }m
                                                                       1 are
                                          (m)      pls
     linear in the original xj , so is ŷ     = Xβ̂ (m). These linear coeffi-
...
where S is the sample covariance matrix of the xj . The conditions αT Svℓ =
0 ensures that zm = Xα is uncorrelated with all the previous linear com-
binations zℓ = Xvℓ . The mth PLS direction ϕ̂m solves:

                       maxα Corr2 (y, Xα)Var(Xα)                               (3.64)
            subject to ||α|| = 1, αT Sϕ̂ℓ = 0, ℓ = 1, . . . , m − 1.
...
since the ϕ̂mj are zero for m > 1 (Exercise 3.14). It can also be shown that
...
restricted multivariate regression problem:
                            N
                            X
       B̂rr (m) = argmin          (yi − BT xi )T Σ−1 (yi − BT xi ).     (3.68)
                  rank(B)=m i=1

With Σ replaced by the estimate YT Y/N , one can show (Exercise 3.21)
that the solution is given by a CCA of Y and X:
                            B̂rr (m) = B̂Um U−
                                             m,                         (3.69)
where Um is the K × m sub-matrix of U consisting of the first m columns,
and U is the K × M matrix of left canonical vectors u1 , u2 , . . . , uM . U−
                                                                            m
is its generalized inverse. Writing the solution as
                   B̂rr (M ) = (XT X)−1 XT (YUm )U−
                                                  m,                    (3.70)
...
are given by
                    Ŷrr (m) = X(XT X)−1 XT YUm U−
                                                 m
                                                                        (3.71)
                             = HYPm ,
where H is the usual linear regression projection operator, and Pm is the
rank-m CCA response projection operator. Although a better estimate of
Σ would be (Y−XB̂)T (Y−XB̂)/(N −pK), one can show that the solution
...
where cm is the mth canonical correlation coefficient. Note that as the ratio
...
where Aλ = X(XT X + λI)−1 XT is the ridge regression shrinkage operator,
...
### Algorithm 3.4: Incremental Forward Stagewise Regression—FSǫ .
     ...
     3. Update βj ← βj + δj , where δj = ǫ · sign[hxj , ri] and ǫ > 0 is a small
        step size, and set r ← r − δj xj .
...
shows the infinitesimal version FS0 obtained letting ǫ → 0. This profile was fit by
...
the prostate data with step size ǫ = 0.01. If δj = hxj , ri (the least-squares
...
### Algorithm 3.2b: Least Angle Regression: FS0 Modification.
     ...
     4. Find the new direction by solving the constrained least squares prob-
        lem
                   min ||r − XA b||22 subject to bj sj ≥ 0, j ∈ A,
                      b
...
methods—LAR, lasso, and FS0 —give identical profiles. If the profiles are
...
a criterion. The answer is more complex than for lasso; the FS0 coefficient
...
The grouped-lasso minimizes the convex criterion
                             ...
                             X                X √
                                        2
           minp ||y − β0 1 −    Xℓ βℓ ||2 + λ     pℓ ||βℓ ||2 ,        (3.80)
         β∈IR
                              ℓ=1             ℓ=1
            √
...
L2 norms ||η||K = (η T Kη)1/2 , as well as allowing overlapping groups of
...
of the form
        ...
        maxc ||xTj XS (XS T XS )−1 ||1 ≤ (1 − ǫ) for some ǫ ∈ (0, 1].          (3.81)
        j∈S
...
ficients for the columns of XS c on XS are not too large, that is, the “good”
...
penalize large coefficients less. For SCAD we use λ = 1 and a = 4, and ν = 12 in
the last panel.
...
makes the computation much more difficult. The adaptive lasso (Zou, 2006)
                                      Pp
uses a weighted penalty of the form j=1 wj |βj | where wj = 1/|β̂j |ν , β̂j is
...
norm. Denote by β̃k (λ) the current estimate for βk at penalty parameter

λ. We can rearrange (3.52) to isolate βj ,
                  N
                                                    !2
                1X         X                                  X
R(β̃(λ), βj ) =       yi −   xik β̃k (λ) − xij βj        +λ          |β̃k (λ)| + λ|βj |,
                2 i=1
                            k6=j                              k6=j
                                                                      (3.83)
...
venience. This can be viewed as a univariate lasso problem with response
                                      (j)          P
variable the partial residual yi − ỹi = yi − k6=j xik β̃k (λ). This has an
explicit solution, resulting in the update
                                    N
                                                         !
                                   X               (j)
                      β̃j (λ) ← S      xij (yi − ỹi ), λ .           (3.84)
                                   i=1
...
Ex. 3.2 Given data on two variables P X and Y , consider fitting a cubic
                                       3
polynomial regression model f (X) = j=0 βj X j . In addition to plotting
the fitted curve, you would like a 95% confidence band about the curve.
Consider the following two approaches:
     1. At each point
                   P3x0 , form a 95% confidence interval for the linear func-
        tion aT β = j=0 βj xj0 .
...
Ex. 3.3 Gauss–Markov theorem:
 ...
 (b) The matrix inequality B  A holds if A − B is positive semidefinite.
     ...
     linear unbiased estimate, then V̂  Ṽ.
...
lem is equivalent to the problem
                  (N              p                     p
                   ...
                   X             X                 2  X    2
    β̂ c = argmin      yi − β0c −   (xij − x̄j )βjc + λ   βjc .     (3.85)
             βc     i=1             j=1                      j=1

                                          c
Give the correspondence between β and the original β in (3.41). Char-
...
of the posterior distribution, under a Gaussian prior β ∼ N (0, τ I), and
Gaussian sampling model y ∼ N (Xβ, σ 2 I). Find the relationship between
the regularization parameter λ in the ridge formula, and the variances τ
and σ 2 .
Ex. 3.7 Assume yi ∼ N (β0 + xTi β, σ 2 ), i = 1, 2, . . . , N , and the parameters
βj , j = 1, . . . , p are each distributed as N (0, τ 2 ), independently of one
another. Assuming σ 2 and τ 2 are known, and β0 is not governed by a
prior (or has a flat improper prior), show that the (minus) log-posterior
                                     PN                  P             2
                                                                            Pp    2
density of β is proportional to        i=1 (yi − β0 −        j xij βj ) + λ  j=1 βj
             2      2
where λ = σ /τ .
...
the centered matrix X with p additional rows λI, and augment y with p
...
Ex. 3.13 Derive the expression (3.62), and show that β̂ pcr (p) = β̂ ls .
...
least squares.
Ex. 3.19 Show that kβ̂ ridge k increases as its tuning parameter λ → 0. Does
...
a generalized SVD problem. Show that the solution is given by u1 =
         1                       1
(YT Y)− 2 u∗1 , and v1 = (XT X)− 2 v1∗ , where u∗1 and v1∗ are the leading left
and right singular vectors in
                           1                     1
                 (YT Y)− 2 (YT X)(XT X)− 2 = U∗ D∗ V∗ T .                   (3.87)
...
(3.68), with Σ estimated by YT Y/N , is given by (3.69). Hint: Transform


                  1
Y to Y∗ = YΣ− 2 , and solved in terms of the canonical vectors u∗m . Show
             1                                                1
that Um = Σ− 2 U∗m , and a generalized inverse is U−     ∗ T
                                                   m = Um Σ .
                                                              2

Ex. 3.22 Show that the solution in Exercise 3.21 does not change if Σ is
estimated by the more natural quantity (Y − XB̂)T (Y − XB̂)/(N − pK).
...
has identical absolute correlation with the response:
                           1
                             |hxj , yi| = λ, j = 1, . . . , p.
                           N
Let β̂ be the least-squares coefficient of y on X, and let u(α) = αXβ̂ for
...
Ex. 3.27 Lasso and LAR:
                    P Consider
                            P the lasso problem in Lagrange multiplier
form: with L(β) = 21 i (yi − j xij βj )2 , we minimize
                                   X
                          L(β) + λ     |βj |                   (3.88)
                                      j
 ...
 (a) Setting βj = βj+ − βj− with βj+ , βj− ≥ 0, expression (3.88) becomes
               P
      L(β) + λ j (βj+ + βj− ). Show that the Lagrange dual function is
                       X                 X          X
              L(β) + λ    (βj+ + βj− ) −    λ+  +
                                             j βj −     λ−  −
                                                         j βj       (3.89)
 ...
 (b) Show that |∇L(β)j | ≤ λ ∀j, and that the KKT conditions imply one
      ...
      Hence show that for any “active” predictor having βj 6= 0, we must
...
Ex. 3.29 Suppose we run a ridge regression with parameter λ on a single
...
Ex. 3.30 Consider the elastic-net optimization problem:
                                                          
               min ||y − Xβ||2 + λ α||β||22 + (1 − α)||β||1 .               (3.91)
                 β
