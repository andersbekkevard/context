...
Throughout this chapter, our training data consists of the $N$ pairs $(x_1, g_1), \ldots, (x_n, g_N)$ where $g_i$ is a class label taking values in $\{1, 2, \ldots, K\}$. Prototype methods represent the training data by a set of points in feature space. These prototypes are typically not examples from the training sample, except in the case of 1-nearest-neighbor classification discussed later. Each prototype has an associated class label, and classification of a query point $x$ is made to the class of the closest prototype. “Closest” is usually defined by Euclidean distance in the feature space, after each feature has
...
(b) If $g_i \ne k$ (i.e., they are in different classes), move the prototype away from the training point:
...
mixture data example. The decision boundary is piecewise linear. The lower panel shows a Gaussian mixture model with a common covariance for all component Gaussians. The EM algorithm for the mixture model was started at the K-means solution. The broken purple curve in the background is the Bayes decision boundary.
...
Figure 13.1. The broken purple curve in the background is the Bayes decision boundary.

**FIGURE 13.4.** k-nearest-neighbors on the two-class mixture data. The upper
panel shows the misclassification errors as a function of neighborhood size. Standard error bars are included for 10-fold cross validation. The lower panel shows the decision boundary for 7-nearest-neighbors, which appears to be optimal for minimizing test error. The broken purple curve in the background is the Bayes decision boundary.

ties $p_k(x), k = 1, \ldots, K$. For $K = 2$ the 1-nearest-neighbor error rate is $2 p_{k^*}(x)(1 - p_{k^*}(x)) \le 2(1 - p_{k^*}(x))$ (twice the Bayes error rate). More generally, one can show (Exercise 13.3)

$$
\sum_{k=1}^{K} p_k(x)(1 - p_k(x)) \le 2(1 - p_{k^*}(x)) - \frac{K}{K-1}(1 - p_{k^*}(x))^2. \tag{13.5}
$$
...
Y &= I\!\left(\operatorname{sign}\!\left\{\prod_{j=1}^{3}\left(X_j - \tfrac{1}{2}\right)\right\} > 0\right); \quad &&\text{problem 2: ``difficult''.}
\end{aligned} \tag{13.6}
$$
                        Y 3                                                       (13.6) 1 Y = I sign        Xj −      > 0 ; problem 2: “difficult.”            2  j=1
...
est-neighbors, K-means (blue) and LVQ (red) over ten realizations for two simulated problems: “easy” and “difficult,” described in the text.

**FIGURE 13.6.** The first four panels are LANDSAT images for an agricultural
area in four spectral bands, depicted by heatmap shading. The remaining two panels give the actual land usage (color coded) and the predicted land usage using a five-nearest-neighbor rule described in the text.
...
and rotated versions of it. The green curve in the middle of the figure depicts this set of rotated “ 3” in 256-dimensional space. The red line is the tangent line to the curve at the original image, with some “ 3”s on this tangent line, and its equation shown at the bottom of the figure.
...
rating class red and green. The vertical strip denotes the 5-nearest-neighbor region using only the horizontal coordinate to find the nearest-neighbors for the target point (solid dot). The sphere shows the 5-nearest-neighbor region using both coordinates, and we see in this case it has extended into the class-red region (and is dominated by the wrong class in this instance).
...
$$
D(x, x_0) = (x - x_0)^T \boldsymbol{\Sigma} (x - x_0), \tag{13.8}
$$
...
$$
\min_{\operatorname{rank}(\mathbf{M})=L} \sum_{i=1}^{N} \operatorname{trace}[(\mathbf{B}_i - \mathbf{M})^2]. \tag{13.11}
$$
...
Ex. 13.1 Consider a Gaussian mixture model where the covariance matrices are assumed to be scalar: $\boldsymbol{\Sigma}_r = \sigma \mathbf{I} \;\; \forall r = 1, \ldots, R$, and $\sigma$ is a fixed parameter. Discuss the analogy between the K-means clustering algorithm and the EM algorithm for fitting this mixture model in detail. Show that in the limit $\sigma \to 0$ the two methods coincide.

Ex. 13.2 Derive formula (13.7) for the median radius of the 1-nearestneighborhood.

Ex. 13.3 Let $E^*$ be the error rate of the Bayes rule in a $K$-class problem, where the true class probabilities are given by $p_k(x), k = 1, \ldots, K$. Assuming the test point and training point have identical features $x$, prove (13.5) K X                                              K pk (x)(1 − pk (x)) ≤ 2(1 − pk∗ (x)) −        (1 − pk∗ (x))2 . K −1 k=1

where $k^* = \arg\max_k p_k(x)$. Hence argue that the error rate of the 1- nearest-neighbor rule converges in $L_1$, as the size of the training set increases, to a value $E_1$, bounded above by           K  E∗ 2 − E∗        .                                (13.12) K −1 [This statement of the theorem of Cover and Hart (1967) is taken from Chapter 6 of Ripley (1996), where a short proof is also given].

Ex. 13.4 Consider an image to be a function $F(x) : \mathbb{R}^2 \to \mathbb{R}^1$ over the twodimensional spatial domain (paper coordinates). Then $F(c + x_0 + A(x - x_0))$ represents an affine transformation of the image $F$, where $A$ is a $2 \times 2$ matrix.
...
3. Using a two-dimensional kernel smoother (Chapter 6), describe how to implement this procedure when the images are quantized to $16 \times 16$ pixels. . , N be square p × p positive semi-definite ma- Ex. 13.5 Let Bi , i = 1, 2, . . P trices Pp     and    let B̄ =  (1/N )      Bi . Write the eigen-decomposition of B̄ as T ℓ=1 θℓ eℓ eℓ with θℓ ≥ θℓ−1 ≥ · · · ≥ θ1 . Show that the best rank-L approximation for the Bi , N X min              trace[(Bi − M)2 ], rank(M)=L i=1 PL           T PN                     2 is given by B̄[L] =     ℓ=1 θℓ eℓ eℓ . (Hint: Write          i=1 trace[(Bi − M) ] as N X                             N X 2 trace[(Bi − B̄) ] +           trace[(M − B̄)2 ]). i=1                           i=1

Ex. 13.6 Here we consider the problem of shape averaging. In particular, $L_i, i = 1, \ldots, M$ are each $N \times 2$ matrices of points in $\mathbb{R}^2$, each sampled from corresponding positions of handwritten (cursive) letters. We seek an affine invariant average $\mathbf{V}$, also $N \times 2$, $\mathbf{V}^T \mathbf{V} = \mathbf{I}$, of the $M$ letters $L_i$ with the following property: $\mathbf{V}$ minimizes

$$
\sum_{j=1}^{M} \min_{A_j} \|L_j - \mathbf{V} A_j\|^2.
$$
...
$$
\sum_{j=1}^{M} \min_{A_j} \|L_j A_j^* - \mathbf{V}\|^2.
$$
