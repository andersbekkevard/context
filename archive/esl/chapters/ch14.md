...
swer ŷi for each xi in the training sample, and the supervisor or “teacher”
provides either the correct answer and/or an error associated with the stu-
dent’s answer. This is usually characterized by some loss function L(y, ŷ),
for example, L(y, ŷ) = (y − ŷ)2 .
...
is relatively large. The intersection of subsets ∩pj=1 (Xj ∈ sj ) is called a
...
to finding subsets of the integers J ⊂ {1, . . . , p}, and corresponding values
v0j , j ∈ J , such that                           
                                  \
                             Pr      (Xj = v0j )                        (14.3)
                                   j∈J
...
Here zik is the value of Zk for this ith case. This is called the “support”
                                                                   Q        or
“prevalence” T (K) of the item set K. An observation i for which k∈K zik =
...
probability of an item set A occurring in a basket, is an abbreviation for
   Q
Pr( k∈A Zk = 1). The “expected confidence” is defined as the support of
the consequent T (B), which is an estimate of the unconditional probability
Pr(B). Finally, the “lift” of the rule is defined as the confidence divided by
...
**FIGURE 14.2.** Market basket analysis: relative frequency of each dummy vari-
                                                                                                 Relative Frequency in Association Rules                         Relative Frequency in Data

                                                                                                       0.0    0.04    0.08    0.12                               0.0   0.02   0.04   0.06
...
We chose the first and third rules based on their high support. The second
...
g0 (x) be a specified probability density function used for reference. For ex-
...
as training data. The resulting estimate µ̂(x) can be inverted to provide an
estimate for g(x)
                                            µ̂(x)
                           ĝ(x) = g0 (x)           .                  (14.12)
                                          1 − µ̂(x)
...
(Section 5.2.1), was fit to the data. Some probability contours of µ̂(x) are
shown in the right panel; these are also the contours of the density estimate
ĝ(x), since ĝ(x) = µ̂(x)/(1 − µ̂(x)), is a monotone function. The contours
roughly capture the data density.
   In principle any reference density can be used for g0 (x) in (14.14). In
practice the accuracy of the estimate ĝ(x) can depend greatly on partic-
...
should be chosen so that the resulting functions µ(x) or f (x) are approx-
imated easily by the method being used. However, accuracy is not always
the primary goal. Both µ(x) and f (x) are monotonic functions of the den-
...
for which the target function µ(x) = E(Y | x) is relatively large. In addition,
...
we define a dissimilarity dj (xij , xi′ j ) between values of the jth attribute,
and then define
                                        p
                                        X
                       D(xi , xi′ ) =         dj (xij , xi′ j )                     (14.20)
                                        j=1
    ...
    of their absolute difference
      ...
      correlation
                 ...
                 ρ(xi , xi′ ) = qP                       P                     ,  (14.22)
                                      ...
                                      j    ij      i       j    i
                   P
      with x̄i = j xij /p. Note that this is averaged over variables,            Pnot ob-
      servations. If the observations are first standardized, then j (xij −
...
weighted average (convex combination)
                             ...
                             X                               p
                                                             X
            D(xi , xi′ ) =         wj · dj (xij , xi′ j );          wj = 1.            (14.24)
                             j=1                              j=1
...
with
                                     N N
                             d¯j = 2      dj (xij , xi′ j )                            (14.25)
                                  N i=1 ′
                                               i =1

being the average dissimilarity on the jth attribute. Thus, the relative in-
fluence of the jth variable is wj · d¯j , and setting wj ∼ 1/d¯j would give all
...
tween observations xi and x′i . This method can fail in the circumstance
...
-2                               •       •                                       •       •
     ...
     -4   -2         0          2              4   6
               ...
               Iteration Number 2                           Iteration Number 20
...
with a Gaussian density having scalar covariance matrix σ 2 I. Then the
...
centers. As the variance σ 2 → 0, these probabilities become 0 and 1, and
...
numbers is regarded as a vector in IR4 . A K-means clustering algorithm
...
Shannon coding theory, we know that in general PK a variable length code
will do better, and the rate then becomes − ℓ=1 pℓ log2 (pℓ )/4. The term
...
for solving (14.35) the computation increases to O(Nk2 ). Given a set of clus-
ter “centers,” {i1 , . . . , iK }, obtaining the new assignments
                              C(i) = argmin dii∗k                        (14.37)
                                      1≤k≤K
...
successive differences in criterion value, WK − WK+1 , at K = K ∗ . That
is, {WK − WK+1 | K < K ∗ } ≫ {WK − WK+1 | K ≥ K ∗ }. An estimate
K̂ ∗ for K ∗ is then obtained by identifying a “kink” in the plot of WK as a
...
expected curve p    minus the observed curve. Shown also are error bars of half-
width s′K = sK 1 + 1/20, where sK is the standard deviation of log WK
...
G(K) is the Gap curve at K clusters, the formal rule for estimating K ∗ is
               ...
               K ∗ = argmin{K|G(K) ≥ G(K + 1) − s′K+1 }.                   (14.39)
                         K
...
LEUKEMIA
                                    ...
                                    MELANOMA                                                                                 NSCLC                                                                                 NSCLC
                                                                                                     OVARIAN                                          OVARIAN                                                                             COLON
                                                                         MELANOMA                                         MELANOMA
                                                                                                                                                                      NSCLC                               BREAST
                                                                                                                                          RENAL          NSCLC        NSCLC                   CNS
                                                                      MELANOMA                                              RENAL
                                                                                                                                         RENAL                             PROSTATE                                                                     COLON
                                                                     MELANOMA                                                            RENAL                           OVARIAN
           LEUKEMIA
                                                                                                                                                                                                                                                        COLON
                                                                                                                                              RENAL                    PROSTATE                                                                            COLON
                                                                MELANOMA
           LEUKEMIA                                                                                                                  RENAL                                                                                                                 COLON
...
correlation coefficient. This is the correlation between the N (N −1)/2 pair-
...
ties {dii′ }. If we define the diameter DG of a group of observations as the
...
complete linkage, dCL (G, H) (14.42) becomes infinite as N → ∞, again
...
526     14. Unsupervised Learning
   ...
   Note that if we flip the orientation of the branches of a dendrogram at any
...
coordinate pair ℓj ∈ Q1 × Q2 . Here Q1 = {1, 2, . . . , q1 }, similarly Q2 , and
...
The “neighbors” of mj are defined to be all mk such that the distance
...
                                            14.4 Self-Organizing Maps      529
   ...
   Notice that distance is defined in the space Q1 ×Q2 of integer topological
...
α and the distance threshold r. Typically α is decreased from say 1.0 to
...
error kx − mj k2 , summed over observations, for both methods. This will
...
panel is the initial configuration, right panel the final one. The 5 × 5 grid of
...
neighbors of mk , or may decrease smoothly with distance kℓk −ℓj k as before.
...
where µ is a location vector in IRp , Vq is a p × q matrix with q orthogonal
...
This leaves us to find the orthogonal matrix Vq :
                       N
                       X
                 min         ||(xi − x̄) − Vq VqT (xi − x̄)||2 .         (14.53)
                 Vq
                       i=1

For convenience we assume that x̄ = 0 (otherwise we simply replace the
observations by their centered versions x̃i = xi − x̄). The p × p matrix
Hq = Vq VqT is a projection matrix, and maps each point xi onto its rank-
q reconstruction Hq xi , the orthogonal projection of xi onto the subspace
...
q principal components (the N rows of the N × q matrix Uq Dq ).
...
by ui1 d1 v1 . Here v1 is the direction of the line and λ̂i = ui1 d1 measures
...
each of these first two principal components ui1 and ui2 , we computed the
...
Here we have displayed the first two principal component directions, v1
...
thonormal p × p matrix4 , and µ a p-vector of location coordinates. Here
||X||2F = trace(XT X) is the squared Frobenius matrix norm.
   Let x̄1 and x̄2 be the column mean vectors of the matrices, and X̃1 and
X̃2 be the versions of these matrices with the means removed. Consider
the SVD X̃T1 X̃2 = UDVT . Then the solution to (14.56) is given by (Exer-
cise 14.8)
                              R̂ = UVT
                                                                   (14.57)
                               µ̂ = x̄2 − R̂x̄1 ,
...
shapes X′ℓ in preshape space superimposed. The right three panels map the pre-
shape M separately to match each of the original S’s.

                                     L
                                     X
                           min             ||Xℓ Aℓ − M||2F ,              (14.60)
                         {Aℓ }L
                              1 ,M   ℓ=1

where the Aℓ are any p × p nonsingular matrices. Here we require a stan-
...
attractive, and can be computed without iteration (Exercise 14.10):
   1. Let Hℓ = Xℓ (XTℓ Xℓ )−1 XTℓ be the rank-p projection matrix defined
      by Xℓ .
   2. MPis the N ×p matrix formed from the p largest eigenvectors of H̄ =
      1   L
      L   ℓ=1 Hℓ .
...
of dimension 2 or more.
   We will first define principal curves for random variables X ∈ IRp , and
then move to the finite data case. Let f (λ) be a parameterized smooth
...
functions f (λ) = [f1 (λ), f2 (λ), . . . , fp (λ)] and let X T = (X1 , X2 , . . . , Xp ).
Consider the following alternating steps:
         (a)      fˆj (λ)   ←         E(Xj |λ(X) = λ); j = 1, 2, . . . , p,
                                                                                                      (14.62)
         (b)     λ̂f (x)    ←         argminλ′ ||x − fˆ(λ′ )||2 .
  The first equation fixes λ and enforces the self-consistency requirement
...
the arc-length λ̂(X), and the projection in (b) is done for each of the ob-
...
matrix the radial-kernel gram matrix; that is, sii′ = exp(−d2ii′ /c), where
...
pairs of points; specifically a pair (i, i′ ) is in NK if point i is among the
...
not in NK , and draw the graph for this modified similarity matrix.
...
eter c.
  The matrix of edge weights W = {wii′ } from a similarityP     graph is called
the adjacency matrix. The degree of vertex i is gi = i′ wii′ , the sum of
...
to the node degrees gi , for example, L̃ = I − G−1 W.
   Spectral clustering finds the m eigenvectors ZN ×m corresponding to the
...
3rd Smallest
               −0.05
                       ...
                       0         100        200      300   400                                            −0.04       −0.02     0.00     0.02

                                             Index                                                         Second Smallest Eigenvector
...
view this method. Defining P = G−1 W, we consider a random walk on
...
tures hφ(xi ), φ(xi′ )i and finding its eigenvectors. The elements of the mth
component zm (mth column of Z) can be written (up to centering) as
       PN
zim = j=1 αjm K(xi , xj ), where αjm = ujm /dm (Exercise 14.16).
...
first principal component function g1 solves
                   ...
                   max VarT g1 (X) subject to ||g1 ||HK = 1                 (14.66)
                  g1 ∈HK
...
the solution to (14.66) is finite dimensional with representation g1 (x) =
PN
   j=1 cj K(x, xj ). Exercise 14.17 shows that the solution is defined by ĉj =
αj1 , j = 1, . . . , N above. The second principal component function is de-
fined in a similar way, with the additional constraint that hg1 , g2 iHK = 0,
and so on.6
   Schölkopf et al. (1999) demonstrate the use of kernel principal compo-
...
ues of e
       K; this is equivalent to finding the eigenvectors corresponding to the
smallest eigenvalues of
                                        e
                                    I − K.                                  (14.68)

This is almost the same as the Laplacian (14.63), the differences being the
centering of Ke and the fact that G has the degrees of the nodes along the
diagonal.
...
                                                            14.5 Principal Components, Curves and Surfaces                                                    549
                                              ...
                                              Radial Kernel (c=2)                                                         Radial Kernel (c=10)

                             0.10

                                                                                                          0.05

Second Largest Eigenvector                                                   Second Largest Eigenvector
                             0.05
                             ...
                             −0.10
                                     ...
                                     −0.10    −0.06       −0.02       0.02                                        −0.06       −0.02          0.02      0.06

                                              First Largest Eigenvector                                                   First Largest Eigenvector

                                             NN Radial Kernel (c=2)                                                Radial Kernel Laplacian (c=2)

                             0.2

                                                                                                          0.15

Second Largest Eigenvector                                                   Third Smallest Eigenvector
                             0.1
                                                                                                          0.10
                             ...
                             −0.1
                                                                                                          ...
                                                                                                          −0.10
                             −0.2
                                     0.00     0.05       0.10      0.15                                           −0.05     0.00      0.05     0.10   0.15

                                              First Largest Eigenvector                                               Second Smallest Eigenvector
...
procedure of Joliffe et al. (2003) takes the first approach, by solving
                                      Pp
          max v T (XT X)v, subject to j=1 |vj | ≤ t, v T v = 1.       (14.69)
...
solves
                       N
                       X
                 min         ||xi − θv T xi ||22 + λ||v||22 + λ1 ||v||1   (14.70)
                 θ,v
                       i=1
                                             subject to ||θ||2 = 1.

Let’s examine this formulation in more detail.
      • If both λ and λ1 are zero and N > p, it is easy to show that v = θ
        and is the largest principal component direction.
      • When p ≫ N the solution is not necessarily unique unless λ > 0. For
...
minimizes
          ...
          X                             K
                                        X                   K
                                                            X
                ||xi − ΘVT xi ||2 + λ         ||vk ||22 +         λ1k ||vk ||1 ,   (14.71)
          i=1                           k=1                 k=1

subject to ΘT Θ = IK . Here V is a p × K matrix with columns vk and Θ
is also p × K.
   Criterion (14.71) is not jointly convex in V and Θ, but it is convex in
each parameter with the other parameter fixed7 . Minimization over V with
...
using (14.71), taken from Sjöstrand et al. (2007). Here the shape of the
...
algorithm is available.
   8 We thank Rasmus Larsen and Karl Sjöstrand for suggesting this application, and
...
xij , wik , hkj ≥ 0.
  The matrices W and H are found by maximizing
                            ...
                            N X
                            X
                L(W, H) =             [xij log(WH)ij − (WH)ij ].               (14.73)
                            i=1 j=1
...
perpositions are shown to the right of the equality sign. The authors point
  ...
  10 These 7 × 7 arrangements allow for a compact display, and have no structural
...
are “pure,”, or “archetypal.”
  As in (14.72), the N × p data matrix X is modeled as
                                  ...
                                  X ≈ WH                                (14.75)
                                                             Pr
where W is N × r and H is r × p. We assume that wik ≥ 0 and k=1 wik =
...
also assume that
                                H = BX                            (14.76)
                                   PN
where B is r × N with bki ≥ 0 and i=1 bki = 1 ∀k. Thus the archetypes
...
The singular-value decomposition
                             √ X = UDV         (14.54) √
                                                       has a latent variable
representation. Writing S = N U and A = DVT / N , we have X =
                                           T
...
14.7 Independent Component Analysis and Exploratory Projection Pursuit      559
                  ...
                  X1      =       a11 S1 + a12 S2 + · · · + a1p Sp
                  X2      =       a21 S1 + a22 S2 + · · · + a2p Sp
                   ..                  ..                              (14.78)
                    .                   .
                  Xp      =       ap1 S1 + ap2 S2 + · · · + app Sp ,
...
factors, A is a p × q matrix of factor loadings, and the εj are uncorrelated
...
where Dε = diag[Var(ε1 ), . . . , Var(εp )]. The Sℓ being Gaussian and un-
...
560     14. Unsupervised Learning
   ...
   Unfortunately the identifiability issue (14.79) remains, since A and ART
...
of the SVD identify the subspace determined by A.
   Because of the separate disturbances εj for each Xj , factor analysis can
...
Sℓ are independent with unit variance; ICA versions of the factor analysis
...
random vector Y is a natural measure of dependence:
   ...
   The quantity I(Y ) is called the Kullback–Leibler
                                                Qp distance between the
density g(y) of Y and its independence version j=1 gj (yj ), where gj (yj )
is the marginal density of Yj . Now if X has covariance I, and Y = AT X
with A orthogonal, then it is easy to show that
                                  p
                                  X
                I(Y )      =             H(Yj ) − H(X) − log | det A|                (14.84)
                                  j=1
                                  Xp
                           =             H(Yj ) − H(X).                              (14.85)
                                   j=1
...
where G(u) = a1 log cosh(au) for 1 ≤ a ≤ 2. When applied to a sample
...
of orthogonal projections such that the projected data look as far from
                                                                                                                                                                                                            ...
                                                                                                                                                                                                            ICA Components
                                                                                                         oo o o                                                                                                                                          oo oo                                                        o            o ooooo                                o                                       o oo
                                                                                                        o
                                                                                                        oooo     ooooooooooooooo                                                                                                                  o o o ooo oo o oooooo                                          o       o
                                                                                                                                                                                                                                                                                                                                                      oooo oooo ooo                            o ooo ooo  ooooo   oooooo                     o
                                                                                                oooo    o        o
                                                                                                                 o
                                                                                                                 o                                                       o                                                            o                  ooooo   ooooo
                                                                                                         ...
                                                                                                         ooo  oo
                                                                                                             ...
                                                                                                             o   o
                                                                                                                 ...
                                                                                                                 o  o
                                                                                                                    o
                                                                                                                    o
                                                                                                                    o  ooooooooo o o                                                                                                      o      o oo ooooo   oo
                                                                                                                                                                                                                                                               ooo
                                                                                                                                                                                                                                                                ooo oo
                                                                                                                                                                                                                                                                     o
                                                                                                                                                                                                                                                                     o oo
                                                                                            ...
                                                                                            o       o  oo
                                                                                                     ...
                                                                                                     ooo o
                                                                                                        oo o ooooooo     o
                                                                                                                          oo         o                              o o                                       o                                         ooooooo   o
                                                                                                                                                                                                                                                                  o
                                                                                                                                                                                                                                                                   o ooo
                                                                                                                                                                                                                                                                       ooo
                                                                                                                                                                                                                                                                         oo          o
                                                                                                                                                                                                                                                                                                     oo              o            o ooo     ooooo   oooooo      o o                                  ooooo
                                                                                                                                                                                                                                                                                                                                                                                                               o  o
                                                                                                                                                                                                                                                                                                                                                                                                                  o
                                                                                                                                                                                                                                                                                                                                                                                                             oooooooo
                                                                                                                                                                                                                                                                                                                                                                                                                     oo oooo oo o o
                                                                                            ooo ooooo       ooo o ooo                                                                                                                     o o                oooo ooooooooo                                                          o oooo          ooo   o oooo                                       oooo             oo                o
                                                                                                                                                                                                                                                o                                                                                     o oo o                                                                                       oo
                                                                                                    o ooo                                                                                                                                                      o oo                                                                                                                                               o
                                                                                                                                                                                                                                                                                                                                                                                                                  o
                                                                                                               o    o                                                                                                                                              oo o o o                                                                       o       o    o                                           o   o   o
                                                                                                       o oo                                                                                                                                                           o                                                                       ooo                                                      o oo
                                                                                                       oooo o                                                                                                                                                o o o                                                                      o ooooo                                                            oo        o
                                                                                                            o                                                                                                                                              o o oo o                                                                            oo o
                                                                                                                                                                                                                                                                                                                                                                                                                oo     o             o
                  Component                                                                       o o
                                                                                                                         o
                                   ...
                                   oooo   oooooo
                                          o     oo oo
                                                  ooooooo    oooo
                                                             o     oooooooo o
                                                                 ooo                                                                                                                                                                                                                             o                                                                       o                                         o
                       oooo oooo oo   oo oooo  ooo           oooo         ooo
                                                                        ooo                                                                                                                                                                                                 o                                                                                      o                                       o
                        oo  ooo oo
                                 oo
                                 o oooo       oooooooooo
                                                                                                                                                                                                                                                                                                                                                                                               ...
                                                                                                                                                                                                                                                                                                                                                                                               o
                                                                                                                                                                                                                                                                                                                                                                                                           ...
                                                                                                                                                                                                                                                                                                                                                                                                           o oo
                                                                                                                                                                                                                                                                                                                                                                                                                  oo o
                                                                                                                                                                                                                                                                                                                                                                                                                          o
                                                                                                                                                                                                                                                                                                                                                                                                                                         oo o
                                                                                                                                                                                                                                                              oooo               o                                    o                         ooooo o               oo                             o oo ooo o
                                                                                                                                                                                                                                                                     ooo oooooo o                                     oo                 ooo                                                             ooo     ooooo oo o                            o
                                                                                                                                                                                                              o o                      o                o  ooooo
                                                                                                                                                                                                                                                               ooooooo ooo
                                                                                                                                                                                                                                                                         oo  oo ooooo
                                                                                                                                                                                                                                                                                                     o o            o o            oo o     ooo
                                                                                                                                                                                                                                                                                                                                                 ...
                                                                                                                                                                                                                                                                                                                                                 ooooo  oo
                                                                                                                                                                                                                                                                                                                                                         oo oo
                                                                                                                                                                                                                                                                                                                                                             o  ooo
                                                                                                                                                                                                                                                                                                                                                               oo   o oo                       o ooooooooo    oooooooo oooo o o o oooo
                                                                                                                                                                                                                        oo            oo oooooo o      oooooo  oo
                                                                                                                                                                                                                                                               oooooo
                                                                                                                                                                                                                                                                   o  oooooo
                                                                                                                                                                                                                                                                          o oo oo
                                                                                                                                                                                                                                                                                oooo
                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                  o                    oooo o oo  ooooooooooo
                                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                                        oo
                                                                                                                                                                                                                                                                         ooo
                                                                                                                                                                                                                                                                          o  o
                                                                                                                                                                                                                                                                             ooooo
                                                                                                                                                                                                                                                                                 o    oooo o
                                                                                                                                                                                                                                                                                    ooo                           o    oooo ooooo   oo oo  oooo oo
                                                                                                                                                                                                                                                                                                                                                 oo
                                                                                                                                                                                                                                                                                                                                                  oo
                                                                                                                                                                                                                                                                                                                                                   oo o
                                                                                                                                                                                                                                                                                                                                                      oo
                                                                                                                                                                                                                       ...
                                                                                                                                                                                                                       o                                oo       o o
                                                                                                                                                                                                                                                                   o o
                                                                                                                                                                                                                                                                   oooo   oo   oooo                                                      ooooo     oo ooo
                                                                                                                                                                                                                                                                                                                                                        oo o
                                                                                                                                                                                                                                                                                                                                                            ooo o                                              ooo  oo   o oo o                       o
                                                                                                                                                                                                                                       o
                                                                                                                                                                                                                                                o       o          o               o                                              o
                                                                                                                                                                                                                                                                                                                                      oo              o   o                                                     oooo    o          o
                                                                                                                                                                                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                                                                                                                                                                                            o o
                                                                                                                                                                                                                                                                                                                                                                                                                             ...
                                                                                                                                                                                                                                                                                                                                                                                                                             oo oo
                                                                                                                                                                                                                                                                                                                                                                                                                                 ooo
                                                                                                                                                                                                                                                                                                                                                                                                                                   oooo  o oo o o o
                                                                                                                                                                                                                                                                                                                                                                                                                                                        o        o
                                                           oooooo oo                                                                                     o oo                                                                                                                                            oo         o                  oo      oooooo oooo  o
                                                                                                                                                                                                                                                                                                                                                            o
                                                                                                                                                                                                                                                                                                                                                        ooooo oo
                                                                                                                                                                                                                                                                                                                                                               o ooooo oo  o  oo                           o
                                                                                                                                                                                                                                                                                                                                                                                                           ooo
                                                                                                                                                                                                                                                                                                                                                                                                             oo
                                                                                                                                                                                                                                                                                                                                                                                                              oo   o
                                                                                                                                                                                                                                                                                                                                                                                                                   oo o o
                                                                                                                                                                                                                                                                                                                                                                                                                        ooooo ooooo  oo o oo o o
                                              ooooo   oo   oooo oo                                                                                   ooo oooo
                                                                                                                                                  oo ooooo                oo     ooo o                                                                                                                                                      oo
                                                                                                                                                                                                                                                                                                                                        oooooooo
                                                                                                                                                                                                                                                                                                                                                    oooo
                                                                                                                                                                                                                                                                                                                                                     ooo    o o
                                                                                                                                                                                                                                                                                                                                                            oooo  o  o
                                                                                                                                                                                                                                                                                                                                                                     oooo  oo                          ooo ooo
                                                                                                                                                                                                                                                                                                                                                                                                             ooo  o
                                                                                                                                                                                                                                                                                                                                                                                                                 ...
                                                                                                                                                                                                                                                                                                                                                                                                                 o  o
                                                                                                                                                                                                                                                                                                                                                                                                                     o
                                                                                                                                                                                                                                                                                                                                                                                                                     oooo  o oo
                                    oooooo  ooooo ooo oo     oooooooooo o o
                                                             o                                                                          oo ooooooo       oooooo
                                                                                                                                                                  oooooo    oo ooooooo o                                                                                                                                       o                 ooo         ooooo o                                          o     ooooooo o
                                     oooooooooo  oo   oo
                                                     oo  oooo      ooooooo o                                                                            o
                                                                                                                                                        oo  ooo   ooo
                                                                                                                                                                 oo     oooo oooooooo     oo o                                                                                                                                              oo o oo
                                                                                                                                                                                                                                                                                                                                                             o oo                                              o
                                                                                                                                                                                                                                                                                                                                                                                                              o oo       o
                                 o
                                 ooooooo
                                      oo   ooo oo ooo   ooo oo oo
                                                              oo
                                                                  oo
                                                                 oo      oo
                                                                           oooo
                                                                                                                                    oooo   ooo   ooooo  ooo
                                                                                                                                                          ooo  oo oo oo
                                                                                                                                                                     ...
                                                                                                                                                                     o  ooo
                                                                                                                                                                       oo
                                                                                                                                                                        ooooo
                                                                                                                                                                             ooooooooo oooooooo                                                                                                                                           oo                      o                                         o oo o o
                             oooo
                                oo
                                 ooooo  oo     o
                                               o  ooo o
                                                                    ...
                                                                    o   ooo ooo                                                   oo oo
                                                                                                                                              ooooo    o  oo
                                                                                                                                                          o o
                                                                                                                                                            o
                                                                                                                                                             o
                                                                                                                                                                  o o   oo  o  o                                                                                                                                                             oo o                                 o                          ooo oo
                                         ooooo   ooooooo    o       ooooo  o                                                                  o              oo
                                                                                                                                                          ooooo   o oo o            ooooo o
                                  ooooooooo
                                 oo    o
                                      o ooo
                                              o
                                                                                                                                                                                                                             ...
                                                                                                                                                                                                                             oooooooo                                                                                                                                                          o o oooo        o
                                                                                                                                                                                                                                                                                                                                                                                                               o  oo
                                                                                                                                                                                                                                                                                                                                                                                                                ooo oo
                                                                                                                                                                                                                                                                                                                                                                                                                     ...
                                                                                                                                                                                                                                                                                                                                                                                                                     o  ooo
                                                                                                                                                                                                                                                                                                                                                                                                                        o
                                                                                                                                                                                                                                                                                                                                                                                                                      ooo   oooooooooo  oooo oooo                o
                                                                                                                            oo ooooo o                                                                                                                                                                                                                                                             ooo  oo oooo
                                                                                                                                                                                                                                                                                                                                                                                                             ooooooo
                                                                                                                                                                                                                                                                                                                                                                                                                  o
                                                                                                                                                                                                                                                                                                                                                                                                                 oo o
                                                                                                                                                                                                                                                                                                                                                                                                                   oo
                                                                                                                                                                                                                                                                                                                                                                                                                   o oooooo
                                                                                                                                                                                                                                                                                                                                                                                                                       o ooo
                                                                                                                                                                                                                                                                                                                                                                                                                          oooo oo ooo ooo   oo oo       o ooo
                                       oo oooo oo
                                                  oo    oooo                                                                                                                                                                    oooo o                        o                                                                                                                                 o oo oo
                                                                                                                                                                                                                                                                                                                                                                                                      o
                                                                                                                                                                                                                                                                                                                                                                                                      ooo   o   oo
                                                                                                                                                                                                                                                                                                                                                                                                                 oo
                                                                                                                                                                                                                                                                                                                                                                                                                  o ooo
                                                                                                                                                                                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                                                                                                                                                                                            o  oo
                                                                                                                                                                                                                                                                                                                                                                                                                               o oooo
                                                                                                                                                                                                                                                                                                                                                                                                                                 o   o
                                                                                                                                                                                                                                                                                                                                                                                                                                     ooo ooooo           o
                                                                                                                           o oo
                                                                                                                                   oo o o                                        o                                      oooooo oo oo oo oo                                                                                                                                                     oo o
                                                                                                                                                                                                                                                                                                                                                                                                  oo
                                                                                                                                                                                                                                                                                                                                                                                                  o oo o
                                                                                                                                                                                                                                                                                                                                                                                                       o
                                                                                                                                                                                                                                                                                                                                                                                                        oooo
                                                                                                                                                                                                                                                                                                                                                                                                                          ...
                                                                                                                                                                                                                                                                                                                                                                                                                          o
                                                                                                                                                                                                                                                                                                                                                                                                                           ...
                                                                                                                                                                                                                                                                                                                                                                                                                           oooooo
                                                                                                                                                                                                                                                                                                                                                                                                                              o  oooo
                                                                                                                                                                                                                                                                                                                                                                                                                                    oooooooo o      o
                                                                                                                                                                                                                                                                                                                                                                                                                                             o oo o o
                                 oo                     oo   oo                                                                          o              o oo    oooo o                                                          oo o         o o oooooooo                                                                                                                                                ooooo   ooooo ooo
                                                                                                                                                                                                                                                                                                                                                                                                                         ooo
                                                                                                                                                                                                                                                                                                                                                                                                                           ooo  oo o ooo o o o oo o o o
                           o ooooo oo                    oo oooo oooo                                                                    o oo ooooooo              ooo oo                                                                                      oooooooo                                                                                                                                ooo        ooo    oooo
                       oooo   ooooooo ooo          oo ooo             oooo                                                 ooooooooooo                      ooo oooo       ooo oo                                     o o  ooooooooooo         oo
                                                                                                                                                                                                                                               o   ooooooooo
                                                                                                                                                                                                                                                oooo   oooo             oooo                                                                                                                                  o   o o
                                                                                                                                                                                                                                                                                                                                                                                                            oooooo oo          o
                   oooooo
                             oo       o ooo                          oo
                                                           ooooooooooo o                                                    ooooooooo             ooooooooo
                                                                                                                                                          oo  oooooo    ooooo                                                       ooooooooooo   ooooooooo oooo
                                                                                                                                                                                                                                                            o     ooo oooo
                                                                                                                                                                                                                                                                     oo     ooo
                                                                                                                                                                                                                                                                                                                                                                                                 o o       oooooooo
                                                                                                                                                                                                                                                                                                                                                                                                                                                   o    o
                      ooo
                         ...
                         oooooo
                              ooooo
                            ooooooo        oo       o  ooo                                                                                   oooo oo ooooooo  o     ooo
                                                                                                                                                                        ooooo   oo o                                      oo oo        o  ooo   oooo  ooooo     ooo
                                                                                                                                                                                                                                                                   ooo    o
                                                                                                                                                                                                                                                                         oo
                                                                                                                                                                                                                                                                          o oooo o
                                                                                                                                                                                                                                                                           oo
                                                                                                                             ...
                                                                                                                             ooo  oo
                                                                                                                                   ooooooooo
                                                                                                                                          oo    oo      oo    ooooooooooo
                                                                                                                                                                        oooooo
                                                                                                                                                                               oo
                                                                                                                                                                                oooo o                                     ooooooooooo  oo   oo o  oooo
                                                                                                                                                                                                                                                   o
                                                                                                                                                                                                                                                   o ooooo  oo
                                                                                                                                                 ...
                                                                                                                                                 oo     oo    ooooo oo o                                                          o o oooo      o
                                                                                                                                                                                                                                                o
                                                                                                                                                                                                                                                  o
                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                        ooo oo
                                                     oo ooo                                                                                        oo o oo                   o                                                               o oooo oo
                                               o oooooo           oo o                                                                             oooo oooo ooo                             o                                                      ooooooo    oooooooo     o                                                                    ooo       oo
                                            ooooo    o ooooo
                                                    oo             oo o   ooo                                                                    o oooo       ooooo oo                                                                 oooo o                    ooooo      o
                                                                                                                                                                                                                                                                            oooo   ooo                                                      oooo         ooo
                                          oo        ooooo                                                                           oooooooo         ooooooo       ooooooooooo oo
                                                                                                                                                                                                                               ooo o               ooo oo      oo ooo  oooooooo                                                   oooooooo       ooooooooo    o
                                   oooooo    ooooo
                                                  oooooo
                                                  o       ooooo
                                                               ooooooooo    oooo
                                                          ...
                                                          ooooo
                                                          o      oo o
                                                                    oo  ooooooooo                                                     oo oo  ooooo
                                                                                                                                                  oooooooo
                                                                                                                                                        ooo
                                                                                                                                                                      ooooo
                                                                                                                                                                         o         oooo oo                                               oo  o     ooo  ooo
                                                                                                                                                                                                                                                        o oo    ooo   o oo       ooooo  ooooo o                                          oo
                                                                                                                                                                                                                                                                                                                                          ooo   oooo  ooo  oo           o
                                   ooo  ooooo oooo
                                              o
                                             oo
                                                                                                                                                                                                                                              ...
                                                                                                                                                                                                                                              o oooooooo   oo
                                                                                                                                                                                                                                                            ooooo
                                                                                                                                                                                                                                                                   oo
                                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                                    oooo
                                                                                                                                                                                                                                                                       ooo ooooo    oooooooo
                                                                                                                                                                                                                                                                                           oo                                  oo    oooo   oooo    oo ooo  oooooo o o
                                      o     o
                                           oo    oooo   oo          ooo                                                                          o        oo                oo oooooo                                                          oo    oo     o    oo    o  oo   oo ooo                                     ooo       o
                                                                                                                                                                                                                                                                                                                                   ooo    oo   o
                              o
                              o
                               oooo
                                                                                                                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                                                                                                                ooo  ooooooooo
                                                                                                                                                                                                                                                                                                                                                                oo
                                                                                                                                                                                                                                                                                                                                                                    oooooo
                                                                                                                                                                                                                                                                                                                                                                      o    oooooooooo
                                                                                                                                                                                                                                                                                                                                                                         ooo        o oo
                                      o ooo   ooooooo        ooo     ooooooo   o                                                         oo
                                                                                                                                          oooooooo  ooooo  o           oooo
                                                                                                                                                                 oooooooo       o oo                                                               o
                                                                                                                                                                                                                                                   oooo          oooooooooooo                                      ooooooo    o
                                                                                                                                                                                                                                                                                                                              oooo  o o  oo
                                                                                                                                                                                                                                                                                                                                          ooo ooo
                                                                                                                                                                                                                                                                                                                                              o           o          o   ooo
                                   oooooo
...
which record brain activity at different locations on the scalp. Figure 14.4111
...
The colored images represent the estimated unmixing coefficient vectors âj
...
Yj = aTj X, a normalized linear combination of the components of X. In
...
X = AS is
                              ...
                              XXp
                                                                        
        ℓ(A, {gj }p1 ; X) =                 log φj (aTj xi ) + gj (aTj xi ],   (14.90)
                              i=1 j=1
...
X      1 X                              
                     T             T              gj (t)           ′′′   2
              log φ(aj xi ) + gj (aj xi ) − φ(t)e        dt − λj {gj (t)} (t)dt .
j=1
      N i=1
                                                                    (14.91)
...
Silverman (1986, Section 5.4.4):
                                               R
    • The first enforces the density constraint φ(t)eĝj (t) dt = 1 on any
      solution ĝj .
   • The second is a roughness penalty, which guarantees that the solution
     ĝj is a quartic-spline with knots at the observed values of sij = aTj xi .

It can further be shown that the solution densities fˆj = φeĝj each have
...
### Algorithm 14.3: Product Density ICA Algorithm: ProDenICA
  ...
  2. Alternate until convergence of A:
   ...
   We fit the functions gj and directions aj by optimizing (14.91) in an
...
convenience we extract one of the p separate problems,
            N                         Z                           Z
      1 X
            [log φ(si ) + g(si )] −       φ(t)e   g(t)
                                                         dt − λ       {g ′′′ (t)}2 (t)dt.   (14.92)
      N i=1
...
a fine grid of L values s∗ℓ in increments ∆ covering the observed values si ,
...
approximate (14.92) by
      ...
      X                                                   o   Z
                                                       ∗            2
            yi∗ [log(φ(s∗ℓ )) + g(s∗ℓ )] − ∆φ(s∗ℓ )eg(sℓ ) − λ g ′′′ (s)ds.                 (14.94)
      ℓ=1

This last expression can be seen to be proportional to a penalized Poisson
log-likelihood with response yℓ∗ /∆ and penalty parameter λ/∆, and mean
...
Newton step (Exercise 14.20)
     ...
     1. For each j update
                             
                       aj ← E X ĝj′ (aTj X) − E[ĝj′′ (aTj X)]aj ,                                                           (14.96)

       where E represents expectation w.r.t the sample xi . Since ĝj is a fitted
...
the closeness of the two frames:
                   p   Pp             !      p  Pp              
                1 X      j=1 |rij |       1 X      i=1 |rij |
 d(A0 , A) =                        −1 +                      − 1 , (14.97)
               2p i=1 maxj |rij |        2p j=1 maxi |rij |

where rij = (Ao A−1 )ij . The right panel in Figure 14.42 compares the
...
pairwise distances as well as possible. Notice that the approximation is
 ...
 12 Francis Bach kindly supplied this code, and helped us set up the simulations.
  13 Some authors define stress as the square-root of S ; since it does not affect the
                                                       M
...
the centered inner product sii′ = hxi − x̄, xi′ − x̄i. The problem then is to
minimize
                                      X
         SC (z1 , z2 , . . . , zN ) =   (sii′ − hzi − z̄, zi′ − z̄i)2  (14.100)
                                         i,i′
...
over the zi and an arbitrary increasing function θ. With θ fixed, we min-
imize over zi by gradient descent. With the zi fixed, the method of iso-
tonic regression is used to find the best monotonic approximation θ(dii′ )
...
we give them here.
   ...
   2. We approximate each point by an affine mixture of the points in its
      neighborhood:                 X
                         min ||xi −       wik xk ||2            (14.102)
                              Wik
                                             k∈N (i)
                                                         PN
       over weights wik satisfying wik = 0, k ∈ / N (i), k=1 wik = 1. wik
...
the side effect that 1T Y = 0, and hence the embedding coordinates are
mean centered.
...
of points; specifically a pair (i, i′ ) is in N if point i is among the K-nearest
neighbors of i′ , or vice-versa. Then we construct the stress function
                                         X
         SL (z1 , z2 , . . . , zN ) =          (dii′ − ||zi − zi′ ||)2
                                      (i,i′ )∈N
                                               X
                                         +               w · (D − ||zi − zi′ ||)2 . (14.105)
                                             (i,i′ ) /
...
diagonal elements cj . Introducing the normalization eT p = N (i.e., the
average PageRank is 1), we can write (14.108) as
                                                  
                    p = (1 − d)eeT /N + dLD−1    c   p
                        = Ap                                    (14.109)
...
satisfies
                                                ...
                                                X
            d(w)
             e (xi , xi′ ) = de (zi , zi′ ) =         (zil − zi′ l )2 ,           (14.112)
                                                l=1
...
Ex. 14.2 Consider a mixture model density in p-dimensional feature space,
                               ...
                               g(x) =           πk gk (x),                        (14.114)
                                         k=1
                                                        P
where gk = N (µk , L·σ 2 ) and πk ≥ 0 ∀k with               k πk = 1. Here {µk , πk }, k =
1, . . . , K and σ 2 are unknown parameters.
580     14. Unsupervised Learning

  Suppose we have data x1 , x2 , . . . , xN ∼ g(x) and we wish to fit the mix-
ture model.
...
Ex. 14.7 Derive (14.51) and (14.52) in Section 14.5.1. Show that µ̂ is not
...
ner product matrix with elements hxi − x̄, xj − x̄i. Let λ1 > λ2 > · · · >
λk be the k largest eigenvalues of S, with associated eigenvectors E√k =
(e1 , e2 , . . .√
√               , ek ). Let Dk be a diagonal matrix with diagonal entries λ1 ,
  λ2 , . . . , λk . Show that the solutions zi to the classical scaling problem
(14.100) are the rows of Ek Dk .
Ex. 14.12 Consider the sparse PCA criterion (14.71).
   ...
   2. Show that with V fixed, solving for Θ amounts to a reduced-rank
      version of the Procrustes problem, which reduces to
                 max trace(ΘT M) subject to ΘT Θ = IK ,                   (14.116)
                  Θ
...
helix. In detail, define X1 = cos(s) + 0.1 · Z1 , X2 = sin(s) + 0.1 · Z2 , X3 =
s + 0.1 · Z3 where s takes on 200 equally spaced values between 0 and 2π,
...
that the mth component zm (mth column of Z) can be written (up to
                    PN
centering) as zim = j=1 αjm K(xi , xj ), where αjm = ujm /dm . Show that
the mapping
       PN of a new observation x0 to the mth component is given by
z0m = j=1 αjm K(x0 , xj ).
                                    PN
Ex. 14.17 Show that with g1 (x) = j=1 cj K(x, xj ), the solution to (14.66)
...
                                                                          Exercises     583
       ...
       N                           Z                      Z
   1 X
         [log φ(si ) + g(si )] −       φ(t)eg(t) dt − λ       {g ′′′ (t)}2 (t)dt.   (14.118)
   N i=1

The solution ĝ is a quartic smoothing spline, and can be written as ĝ(s) =
...
tions for θ̂k , k = 1, 2, 3, show that the solution fˆ = φeĝ is a density, and
has
R ′′mean2 zero and variance one. If we used a second-derivative penalty
  {g (t)} (t)dt instead, what simple modification could we make to the
problem to maintain the three moment conditions?

Ex. 14.19 If A is p × p orthogonal, show that the first term in (14.92) on
page 568
                            Xp X
                               N
                                  log φ(aTj xi ),
...
Ex. 14.20 Fixed point algorithm for ICA (Hyvärinen et al., 2001). Consider
                       ...
                       N   p
                             "          r
                                                 !   r
                                                              #
                      XX              X             X
           L(W, H) =          xij log     wik hkj −    wik hkj .
                               i=1 j=1                  k=1                  k=1

      Using the concavity ofPlog(x), show that for any set of r values yk ≥ 0
                               r
      and 0 ≤ ck ≤ 1 with k=1 ck = 1,
                             r
                                   !    r
                           X           X
                     log         yk ≥     ck log(yk /ck )
...
                                                                  Exercises       585
 ...
 (b) Hence show that, ignoring constants, the function
                                  ...
                                  X   r
                                                      asikj                      
        g(W, H | Ws , Hs )    =                 uij           log w ik + log h kj
                                  i=1 j=1 k=1
                                                       bsij
                                           p X
                                         N X
...
rank one case (r = 1).
 ...
 (b) Show that the final iterates have the explicit form
                      Pp                            PN
                        j=1 xij             1             xik
         w i = c · P N Pp         ,   hk = · PN i=1    Pp                   (14.122)
                     i=1        x
                           j=1 ij
                                            c      i=1   j=1 xij
