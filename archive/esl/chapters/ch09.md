...
effects. These methods are called “generalized additive models.”
   In the regression setting, a generalized additive model has the form
     ...
     E(Y |X1 , X2 , . . . , Xp ) = α + f1 (X1 ) + f2 (X2 ) + · · · + fp (Xp ).   (9.1)
...
of a generalized additive model. In general, the conditional mean µ(X) of
...
Examples of classical link functions are the following:
      • g(µ) = µ is the identity link, used for linear and additive models for
        Gaussian response data.
      • g(µ) = logit(µ) as above, or g(µ) = probit(µ), the probit link function,
...
tion fˆj can then reveal possible nonlinearities in the effect of Xj . Not all
...
Thus each of the following would qualify:
    • g(µ) = X T β + αk + f (Z)—a semiparametric model, where X is a
      ...
      nonparametrically.
    • g(µ) = f (X) + gk (Z)—again k indexes the levels of a qualitative
      input V , and thus creates an interaction term g(V, Z) = gk (Z) for
      the effect of V and Z.
    • g(µ) = f (X) + g(Z, W ) where g is a nonparametric function in two
      features.
...
where St is a seasonal component, Tt is a trend and ε is an error term.
...
where the error term ε has mean zero. Given observations xi , yi , a criterion
like the penalized sum of squares (5.9) of Section 5.4 can be specified for
this problem,
                                   N                p
                                                                      !2       p          Z
                                   X                X                          X               ′′
PRSS(α, f1 , f2 , . . . , fp ) =         yi − α −         fj (xij )        +         λj       fj (tj )2 dtj ,
                                   i=1              j=1                        j=1
                                                                     (9.7)
...
### Algorithm 9.1: The Backfitting Algorithm for Additive Models.
                       PN
 1. Initialize: α̂ = N1 1 yi , fˆj ≡ 0, ∀i, j.

 2. Cycle: j = 1, 2, . . . , p, . . . , 1, 2, . . . , p, . . . ,
                                           "                                  #
                                                               X
                        fˆj ← Sj {yi − α̂ −                      fˆk (xik )} ,
                                                                         1
                                                                            N
        ...
        until the functions fˆj change less than a prespecified threshold.
...
can add or subtract any constants to each of the functions fj , and adjust
                                                                   PN
α accordingly. The standard convention is to assume that 1 fj (xij ) =
...
We set α̂ = ave(yi ), and it never changes. We apply a cubic smoothing
                                         P
spline Sj to the targets {yi − α̂ − k6=j fˆk (xik )}N   1 , as a function of xij ,
                               ˆ
to obtain a new estimate fj . This is done for each predictor in turn, using
the
P current      estimates of the other functions fˆk when computing yi − α̂ −
        ˆ                                                        ˆ
   k6=j fk (xik ). The process is continued until the estimates fj stabilize. This
...
a popular application.
   The generalized additive logistic model has the form
                 Pr(Y = 1|X)
           log               = α + f1 (X1 ) + · · · + fp (Xp ).          (9.8)
                 Pr(Y = 0|X)
  The functions f1 , f2 , . . . , fp are estimated by a backfitting algorithm
...
### Algorithm 9.2: Local Scoring Algorithm for the Additive Logistic Regression Model.
  1. Compute starting values: α̂ = log[ȳ/(1 − ȳ)], where ȳ = ave(yi ), the
     sample proportion of ones, and set fˆj ≡ 0 ∀j.
                      P
  2. Define η̂i = α̂ + j fˆj (xij ) and p̂i = 1/[1 + exp(−η̂i )].
        Iterate:
...
the smoothing-spline parameter λj was chosen so that trace[Sj (λj )]−1 = 4,
...
TABLE 9.2. Significant predictors from the additive model fit to the spam train-
ing data. The coefficients represent the linear part of fˆj , along with their standard
errors and Z-score. The nonlinear P-value is for a test of nonlinearity of fˆj .
...
fˆ(our)                                                   fˆ(over)
             ...
             -5                                                          -5
                   ...
                   0   2        4       6       8                             0       1            2       3                              0         2         4           6                              0   2        4    6     8    10
...
fˆ(free)
             ...
             -5                                                          -5
                   ...
                   0   2        4   6       8        10                       0       2            4       6                              0     5        10          15        20                        0            5          10
...
fˆ(george)
             ...
             -10
                                                                                                                                    -10
                   0       10       20          30                            0       2            4           6                          0     5        10       15          20                         0        5         10        15
...
fˆ(ch!)                                                   fˆ(ch$)
             ...
             -5                                                                                                                                                                                    -5
                   ...
                   0       10       20          30                            0   1       2    3       4   5       6                      0   2000            6000            10000                      0       5000       10000     15000
...
right panel of Figure 9.2, we first split at X1 = t1 . Then the region X1 ≤ t1
...
f (xi ))2 , it is easy to see that the best ĉm is just the average of yi in region
Rm :

                              ĉm = ave(yi |xi ∈ Rm ).                            (9.11)
...
split point s, and define the pair of half-planes
         ...
         R1 (j, s) = {X|Xj ≤ s} and R2 (j, s) = {X|Xj > s}.                       (9.12)

  Then we seek the splitting variable j and split point s that solve
         h       X                          X                 i
      min min           (yi − c1 )2 + min          (yi − c2 )2 .     (9.13)
        j, s   c1                               c2
                    xi ∈R1 (j,s)                     xi ∈R2 (j,s)

For any choice j and s, the inner minimization is solved by

      ĉ1 = ave(yi |xi ∈ R1 (j, s)) and ĉ2 = ave(yi |xi ∈ R2 (j, s)).            (9.14)
...
large tree is pruned using cost-complexity pruning, which we now describe.
   We define a subtree T ⊂ T0 to be any tree that can be obtained by
...
we define the cost complexity criterion
                                |T |
                                X
                    Cα (T ) =          Nm Qm (T ) + α|T |.                (9.16)
                                m=1

The idea is to find, for each α, the subtree Tα ⊆ T0 to minimize Cα (T ).
...
minimizes Cα (T ). To find Tα we use weakest link pruning: we successively
collapse
P        the internal node that produces the smallest per-node increase in
...
vations in node m to class k(m) = arg maxk p̂mk , the majority class in
node m. Different measures Qm (T ) of node impurity include the following:
                                                  1
                                                           P
   Misclassification error:                      Nm    i∈Rm I(yi 6= k(m)) = 1 − p̂mk(m) .
                                                 P                   PK
   Gini index:                                    k6=k′ p̂mk p̂mk′ =   k=1 p̂mk (1 − p̂mk ).
                                                  PK
   Cross-entropy or deviance:                    − k=1 p̂mk log p̂mk .
                                                                        (9.17)
...
them to class k withPprobability p̂mk . Then the training error rate of this
rule in the node is k6=k′ p̂mk p̂mk′ —the Gini index. Similarly, if we code
each observation as 1 for class k and zero otherwise, the variance over the
node of this 0-1 response is p̂mk (1 − p̂mk ). Summing over classes k again
...
that is, Lkk = 0 ∀k. To incorporatePthe losses into the modeling process,
we could modify the Gini index to k6=k′ Lkk′ p̂mk p̂mk′ ; this would be the
...
p̂mk p̂mk′ is Lkk′ + Lk′ k . For two classes a better approach is to weight the
...
alter the prior probability on the classes. In a terminal node,P  the empirical
Bayes rule implies that we classify to class k(m) = arg mink ℓ Lℓk p̂mℓ .
...
Rather than restricting splits to be of thePform Xj ≤ s, one can allow splits
along linear combinations of the form        aj Xj ≤ s. The weights aj and
...
ture. In regression, suppose, for example, that Y = c1 I(X1 < t1 )+c2 I(X2 <
...
are used to characterize a rule. They are defined as follows:
 ...
 Specificity: probability of predicting non-disease given true state is non-
     disease.
                                                       ...
                                                       email                                          spam                       spam             email
                                                      180/1065                                        9/112                      26/337            0/22

                                        ch!<0.191                                        george<0.15        CAPAVE<2.907
                                                                 ch!>0.191                      george>0.15      CAPAVE>2.907

                                  email                                      email            spam            email       spam            spam
                                  80/861                                    100/204           6/109            0/3        19/110          7/227

                          george<0.005                         CAPAVE<2.7505                                         1999<0.58
                                 george>0.005                       CAPAVE>2.7505                                           1999>0.58

                            email            email                   email            spam                           spam        email
                            80/652           0/209                   36/123           16/81                          18/109        0/1

                       hp<0.03                           free<0.065
                              hp>0.03                            free>0.065

                       email      email                      email            spam
                       77/423        3/229                   16/94            9/29

           CAPMAX<10.5       business<0.145
                    CAPMAX>10.5      business>0.145

       email                      email              email           spam
       20/238                     57/185             14/89            3/5

receive<0.125         edu<0.045
        receive>0.125        edu>0.045

 email          spam        email            email
  19/236         1/2        48/113            9/72
...
in
p Figure 9.6. The standard error of each curve near 0.9 is approximately
...
similar to CART. PRIM is designed for regression (quantitative response
                        ...
                        1                                             2                                           3                                               4
                oo           o     oooo o                         oo           o     oooo o                   oo           o     oooo o                       oo           o     oooo o
  oo                                                oo                                          oo                                              oo
 o o o o oo ooo o oooo oo o                        o o o o oo ooo o oooo oo o                  o o o o oo ooo o oooo oo o                      o o o o oo ooo o oooo oo o
     oo oo ooo          oo ooo         o               oo oo ooo          oo ooo         o         oo oo ooo          oo ooo         o             oo oo ooo          oo ooo         o
   oo
           oo oo o       o o o      oo
                                    oo               oo
                                                             oo oo o       o o o      oo
                                                                                      oo         oo
                                                                                                         oo oo o       o o o      oo
                                                                                                                                  oo             oo
                                                                                                                                                         oo oo o       o o o      oo
                                                                                                                                                                                  oo
        o        oo        o             o                o        oo        o             o          o        oo        o             o              o        oo        o             o
   o oo oo o o o              o ooooo ooo
                                      o              o oo oo o o o              o ooooo ooo
                                                                                        o        o oo oo o o o              o ooooo ooo
                                                                                                                                    o            o oo oo o o o              o ooooo ooo
                                                                                                                                                                                    o
           o
           o                                                 o
                                                             o                                           o
                                                                                                         o                                               o
                                                                                                                                                         o
 oo o oo o oo ooo o                oo
                                        oo         oo o oo o oo ooo o                oo
                                                                                          oo   oo o oo o oo ooo o                oo
                                                                                                                                      oo       oo o oo o oo ooo o                oo
                                                                                                                                                                                      oo
    o       oo o                       o              o       oo o                       o        o       oo o                       o            o       oo o                       o
                                     o                                                 o                                           o                                               o
          o       oo o o                                    o       oo o o                              o       oo o o                                  o       oo o o
 o      o o o oooo o o  oo o oo o o                o      o o o oooo o o  oo o oo o o          o      o o o oooo o o  oo o oo o o              o      o o o oooo o o  oo o oo o o
      o      o        o o     o oo o o                  o      o        o o     o oo o o            o      o        o o     o oo o o                o      o        o o     o oo o o
  o
         ooooo o o o o o o                          o
                                                           ooooo o o o o o o                    o
                                                                                                       ooooo o o o o o o                        o
                                                       ...
                                                       o    o                    o o               o
                                                                                               o o ooo ooo o o o oo
                                                                                                   o    o                    o o                   o
                                                                                                                                               o o ooo ooo o o o oo
                                                                                                                                                   o    o                    o o
  oo o          o  o   o      o o                   oo o          o  o   o      o o             oo o          o  o   o      o o                 oo o          o  o   o      o o
                     o o                                               o o                                         o o                                             o o
...
Therefore, the collection of basis functions is
                         C = {(Xj − t)+ , (t − Xj )+ } t ∈ {x1j , x2j , . . . , xN j }                                (9.18)
                                                                                 j = 1, 2, . . . , p.
...
function hm in the model set M with one of the reflected pairs in C. We
add to the model M the term of the form
       β̂M +1 hℓ (X) · (Xj − t)+ + β̂M +2 hℓ (X) · (t − Xj )+ , hℓ ∈ M,
...
that produces the largest decrease in training error. Here β̂M +1 and β̂M +2
...
choice is β̂1 (X2 − x72 )+ + β̂2 (x72 − X2 )+ . Then this pair of basis functions
...
estimated best model fˆλ of each size (number of terms) λ. One could use
...
extraneous terms involving other predictors.
   Let µ(x) be the true mean of Y , and let
                   ...
                   MSE0      =   avex∈Test (ȳ − µ(x))2 ,
                                                                       (9.23)
                   MSE       =   avex∈Test (fˆ(x) − µ(x))2 .
...
tion we assume that the first element of xi is one, to account for intercepts.
   Here is how an HME is defined. The top gating network has the output
                                        T
                                   e γj x
                  gj (x, γj ) = PK                      , j = 1, 2, . . . , K,                   (9.25)
                                                γkT x
                                  k=1 e

where each γj is a vector of unknown parameters. This represents a soft
...
probability that Y = y is
                        ...
                        X                   K
                                            X
         Pr(y|x, Ψ) =         gj (x, γj )         gℓ|j (x, γjℓ )Pr(y|x, θjℓ ).   (9.30)
                        j=1                 ℓ=1

This is a mixture model, with the mixture probabilities determined by the
gating network models.
PTo estimate the parameters, we maximize the log-likelihood of the data,
...
and ∆ℓ|j given the current values of the parameters. These expectations
...
each M-step: N p2 for the regressions, and N p2 K 2 for a K-class logistic
...
## Exercises
Ex. 9.1 Show that a smoothing spline fit of yi to xi preserves the linear
...
variance σ 2 , and aP  fitting operation y → ŷ, let’s define the degrees of
freedom of a fit by i cov(yi , ŷi )/σ 2 .
...
X1 , X2 , . . . , Xp .
 ...
 (c) Generate response values also as standard Gaussian (σ 2 = 1), indepen-
 ...
 (e) If the regression tree fit were a linear operation, we could write ŷ = Sy
