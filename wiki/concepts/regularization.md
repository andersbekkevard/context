...
- [[L20-boosting-2]]: boosting itself uses **multiple regularization levers**: shrinkage rate $\nu \le 0.1$ ("forces weak learners by not letting them have a strong vote"), [[stochastic-gradient-boosting|stochastic subsampling]], early stopping, and (for XGBoost) explicit L1/L2 on leaf weights, pruning rate $\gamma$, and [[nn-regularization|dropout]].
...
- [[nn-regularization|dropout]]: random ensembling inside a network
