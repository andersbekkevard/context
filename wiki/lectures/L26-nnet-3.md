...
— bias $b$, weighted sum of the current input, plus a weighted sum of the *previous hidden state*. The carry-over is how sequence information is propagated.
...
The prof's preferred framing — what's *really* going on past the interpolation point:
