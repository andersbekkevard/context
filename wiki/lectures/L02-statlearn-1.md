...
His critique of the plotting choice: x-axes should really be **money spent** on ads, not number of ads, since that's the decision variable — the slope then becomes "benefit per dollar spent" and you can compare across media. Newspaper has the noisiest fit (the εᵢ's are big) but if newspaper ads are basically free, you'd still buy them. Foreshadows that **slope size, fit quality, and the practical decision interact** — the same trio that becomes statistical-vs-practical-significance in L05.
