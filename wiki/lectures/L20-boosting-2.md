...
But we're not free to choose any function — $f_m$ has to be a sum of trees. So we do the closest thing: **fit a tree to the negative gradient by least squares.**
...
Outlier scenario he uses: sensors recording $y$ and $x$, someone hits the table, three or four $y$ values are huge. Squared loss "is going to push hard" on these — your model bends to chase the noise.
...
XGBoost uses a Taylor-series expansion to second order — keeps both gradient (direction) and Hessian (step size). Same algorithm structure, second-order $\tilde L$.
...
Brain analogy: get hit on the head, lose some neurons, but you can still think — because the brain has redundancy, no single neuron carries a memory. Dropout forces the same redundancy in the model. In boosting, this means later trees can take over what early trees did, so no early tree dominates.
