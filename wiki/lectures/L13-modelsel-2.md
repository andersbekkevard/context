...
- $\lambda \to \infty$: all $\hat\beta_j \to 0$. **Lowest variance (zero — perfect repeatability!), worst bias.**
...
Beyond "ridge doesn't go to zero": ridge **encourages ties / combinations**. If $\beta_1$ and $\beta_2$ are correlated (solve roughly the same problem), lasso picks one and zeros the other. Ridge averages over them — keeps both at moderate values.
