...
1. The denominator doesn't depend on $k$, so drop it.
2. We only need $\arg\max_k$, not the actual probability.
3. $\log$ is monotone, so $\arg\max$ is unchanged.
...
> The Gaussian density has an $x^2$ inside the exponent. In LDA, the coefficient on $x^2$ is $-1/(2\sigma^2)$, **the same for every $k$** because we pooled the variance. So the $x^2$ contribution is identical across classes and drops out of the $\arg\max$. **In QDA we don't pool**, so the coefficient becomes $k$-dependent, the $x^2$ term survives, and the discriminant becomes quadratic.
