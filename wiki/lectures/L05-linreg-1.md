...
- **Squared rectangles**: each εᵢ becomes a square of side εᵢ. The fit minimizes the **total area** of all those squares. This makes the outlier sensitivity geometric: a big residual contributes its area, which scales quadratically. With absolute deviation you'd be minimizing total *line lengths*, scaling linearly — much less pull from a single far-away point.
